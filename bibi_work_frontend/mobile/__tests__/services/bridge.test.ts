...
describe('BridgeService', () => {
  ...
  describe('emit', () => {
    it('sends a fire-and-forget message via WebSocket', () => {
      ...
      expect(mockSend).toHaveBeenCalledWith(
        'subscribe-chat:send',
        expect.objectContaining({ id: expect.any(String), data: { text: 'hello' } }),
      );
    });
    ...
    it('sends without data', () => {
      ...
      expect(mockSend).toHaveBeenCalledWith(
        'subscribe-ping',
        expect.objectContaining({ id: expect.any(String), data: undefined }),
      );
    });
  });
  ...
  describe('request', () => {
    it('sends request and resolves when server responds', async () => {
      ...
      const request = mockSend.mock.calls[0][1];
      ...
      simulateMessage(`subscribe.callback-get:sessions${request.id}`, [{ id: 1 }]);
    ...
    it('sends request with data', async () => {
      ...
      simulateMessage(`subscribe.callback-create:session${request.id}`, { id: 2 });
    ...
    it('cleans up listener after response', async () => {
      ...
      const request = mockSend.mock.calls[0][1];
      simulateMessage(`subscribe.callback-cleanup-test${request.id}`, 'result');
      await promise;
