...
function getStepItems(msg: TMessage, translate: (key: string) => string): StepItem[] {
  if (msg.type === 'tool_group' && Array.isArray(msg.content)) {
    return msg.content.map((t: any) => ({
      name: t.description || t.name || translate('chat.toolCall'),
  ...
  if (msg.type === 'tool_call') {
    return [
      {
        name: msg.content?.name || translate('chat.toolCall'),
  ...
  if (msg.type === 'acp_tool_call') {
    ...
    return [
      {
        name: update?.title || update?.kind || translate('chat.toolCall'),
  ...
  if (msg.type === 'codex_tool_call') {
    return [
      {
        name: msg.content?.title || msg.content?.description || msg.content?.kind || translate('chat.toolCall'),
        status: normalizeStatus(msg.content?.status),
      },
    ];
  }
  return [{ name: translate('chat.toolCall'), status: 'pending' }];
}
