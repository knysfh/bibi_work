...
export default ({ config }: ConfigContext): ExpoConfig => {
  return {
    ...
    scheme: 'biwork-mobile',
    userInterfaceStyle: 'automatic',
    ios: {
      supportsTablet: true,
      bundleIdentifier: 'ai.resopod.biwork',
      buildNumber: String(VERSION.buildNumber),
      infoPlist: {
        ITSAppUsesNonExemptEncryption: false,
        NSCameraUsageDescription: 'BiWork needs camera access to scan QR codes for server login.',
      },
    },
    android: {
      adaptiveIcon: {
