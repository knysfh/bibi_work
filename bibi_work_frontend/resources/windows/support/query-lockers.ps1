...
function New-SelfLockProcess([int]$processId) {
  return [pscustomobject]@{ name = 'BiWork installer'; pid = $processId }
}
...
namespace BiWork.RestartManager {
...
'@
  ...
  $result = [BiWork.RestartManager.Native]::RmStartSession([ref]$sessionHandle, 0, $key)
  if ($result -ne 0) {
  ...
  try {
    for ($i = 0; $i -lt $resources.Count; $i += 256) {
      ...
      $result = [BiWork.RestartManager.Native]::RmRegisterResources($sessionHandle, [uint32]$chunk.Count, $chunk, 0, [IntPtr]::Zero, 0, $null)
      if ($result -ne 0) {
    ...
    for ($attempt = 0; $attempt -lt 6; $attempt++) {
      ...
      $count = [uint32]0
      $reasons = [uint32]0
      $result = [BiWork.RestartManager.Native]::RmGetList($sessionHandle, [ref]$needed, [ref]$count, $null, [ref]$reasons)
    ...
    if ($result -eq $ERROR_MORE_DATA -or $needed -gt 0) {
      for ($attempt = 0; $attempt -lt 6; $attempt++) {
        ...
        $result = [BiWork.RestartManager.Native]::RmGetList($sessionHandle, [ref]$needed, [ref]$count, $apps, [ref]$reasons)
    ...
    [void][BiWork.RestartManager.Native]::RmEndSession($sessionHandle)
  }
} catch {
