...
describe('useTeamRunView', () => {
  ...
  it('reconcile clears stale local state when backend has no active run', async () => {
    ...
    expect(result.current.state.lastTerminalRun).toBeUndefined();
    expect(result.current.state.childTurnsBySlot).toEqual({});
    expect(result.current.state.slotWorkBySlot).toEqual({});
  });

  it('preserves terminal run events for display without keeping them active', async () => {
    teamEventMocks.invoke.getRunState.mockResolvedValue({ active_run: null });
    const { result } = renderHook(() => useTeamRunView('team-1'));
    const runCancelled = teamEventMocks.handlers.runCancelled as TeamRunHandler;
    ...
    expect(result.current.state.activeRun).toBeUndefined();
    expect(result.current.state.lastTerminalRun?.team_run_id).toBe('run-1');
    expect(result.current.state.lastTerminalRun?.status).toBe('cancelled');
    expect(result.current.state.childTurnsBySlot).toEqual({});
    expect(result.current.state.slotWorkBySlot).toEqual({});

    await act(async () => {
      await result.current.reconcile('after-terminal');
    });
  ...
  it('clears the terminal run snapshot when a new active run arrives', () => {
    const { result } = renderHook(() => useTeamRunView('team-1'));
    const runCancelled = teamEventMocks.handlers.runCancelled as TeamRunHandler;
    const runUpdated = teamEventMocks.handlers.runUpdated as TeamRunHandler;
    ...
    act(() => {
      ...
      runUpdated({
        ...
        pending_wake_count: 0,
        starting_child_count: 0,
        slot_work: [{ slot_id: 'lead', role: 'lead', pending_wake_count: 0, starting_child_count: 0 }],
      });
    });
