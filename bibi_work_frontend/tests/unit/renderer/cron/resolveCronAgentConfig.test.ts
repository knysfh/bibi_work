...
describe('resolveCronAgentConfig', () => {
  it('keeps preset acp assistants on their backend slug', () => {
    const result = resolveCronAgentConfig({
      ...
      codexModelRequiredMessage: 'provider required',
    });
  ...
  it('omits backend for non-codex assistants and lets the backend derive runtime identity', () => {
    const result = resolveCronAgentConfig({
...
function agent(_id: string, type: string, backend?: string): AssistantAgent {
  return {
    type,
    source: type === 'codex' ? 'internal' : 'builtin',
    acp_backend: backend,
  };
}
