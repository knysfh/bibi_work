...
describe('channel assistant binding helpers', () => {
  const assistants = [
    assistant({ id: 'bare-codex', name: 'Codex', source: 'generated', preset_agent_type: 'codex' }),
  ...
  it('prefers the generated codex assistant as the default selection', () => {
    expect(getDefaultChannelAssistant(assistants)?.id).toBe('bare-codex');
  });
  ...
  it('falls back to the default assistant only when no binding was saved', () => {
    expect(resolveChannelAssistantId(undefined, assistants)).toBe('bare-codex');
  });
