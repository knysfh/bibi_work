...
describe('start frame', () => {
  ...
  it('includes the shared broker access token in the start frame when present', () => {
    globalThis.__biworkAccessToken = 'stt-token';
    const callbacks = makeCallbacks();
    startSpeechStream({ callbacks, createSocket });
    const sock = lastSocket();
    ...
    expect(sock.sentText[0]).toBe(
      '{"type":"start","format":"pcm16","sampleRate":24000,"channels":1,"accessToken":"stt-token"}'
    );
  });
});
