...
describe('useGuidAssistantSelection', () => {
  ...
  it('restores the last selected guid assistant before falling back to the codex default', async () => {
    mockAssistants = [
  ...
  it('reads codex mode options from the managed agent catalog', async () => {
    mockAssistants = [
      {
        id: 'bare:632f31d2',
        source: 'generated',
        ...
        sort_order: 1,
        agent_id: '632f31d2',
        agent: { type: 'codex', source: 'internal' },
        enabled_skills: [],
        custom_skill_names: [],
        disabled_builtin_skills: [],
        context_i18n: {},
    ...
    expect(result.current.selectedAssistantBackend).toBe('codex');
    expect(result.current.selectedMode).toBe('default');
...
function assistantFixture({
  ...
  const isCodex = runtimeKey === 'codex';
  return {
    id,
    source,
    name: id,
