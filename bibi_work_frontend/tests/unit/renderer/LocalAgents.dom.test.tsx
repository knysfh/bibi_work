...
vi.mock('@/renderer/components/base/BiWorkModal', () => ({ default: () => null }));
...
vi.mock('@renderer/pages/settings/AgentSettings/AgentHubModal', () => ({
  AgentHubModal: ({ visible }: { visible: boolean }) =>
    visible ? <div data-testid='agent-hub-modal'>agent hub</div> : null,
}));
...
describe('LocalAgents', () => {
  it('opens the governed Agent Hub from the management page', () => {
    useManagedAgents.mockReturnValue({ agents: makeAgents(), revalidate: vi.fn(), refreshCatalog: vi.fn() });

    render(<LocalAgents />);
    fireEvent.click(screen.getByTestId('btn-open-agent-hub'));
  ...
  it('renders the setup-guide action for official agents diagnostics', () => {
    ...
    expect(openExternalUrl).toHaveBeenCalledWith('https://github.com/knysfh/bibi_work/wiki/ACP-Setup');
  });

  it('binds assistants to managed agents by agent_id instead of runtime backend', () => {
    const [codexAgent, claudeAgent] = makeAgents();
  ...
  it('renders the availability filter as underline tabs and switches the visible official agents', () => {
    ...
    // Default "all": both official agents visible (Codex online, Claude Code missing).
    expect(screen.getByText('Codex')).toBeInTheDocument();
    expect(screen.getByText('Claude Code')).toBeInTheDocument();
