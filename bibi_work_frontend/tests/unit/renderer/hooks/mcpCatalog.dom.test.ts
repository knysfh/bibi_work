...
import { ensureBackendMcpCatalog, toBackendMcpPayload } from '@/renderer/hooks/mcp/catalog';
...
describe('ensureBackendMcpCatalog', () => {
  ...
  it('preserves streamable HTTP transport and custom headers in backend payloads', () => {
    expect(
      toBackendMcpPayload({
        name: 'secured stream',
        description: 'streamable MCP',
        transport: {
