...
describe('resolveAssistantAvatar', () => {
  ...
  it('rejects legacy backend logo api routes without an asset owner', () => {
    expect(resolveAssistantAvatar('/api/assets/logos')).toEqual({
      kind: 'fallback',
    });
    expect(resolveAssistantAvatar('/api/assets/logos/ai-major/gemini.svg')).toEqual({
      kind: 'fallback',
    });
    expect(resolveAssistantAvatar('http://127.0.0.1:8361/api/assets/logos/ai-major/gemini.svg')).toEqual({
      kind: 'fallback',
    });
  });
  ...
  it('does not derive avatar routes from local absolute assistant asset paths', () => {
    expect(resolveAssistantAvatar('/Users/demo/.biwork/assistant-avatars/custom-1.jpg')).toEqual({
      kind: 'fallback',
    });
  });
  ...
  it('does not expose file urls as image sources without an assistant id', () => {
    expect(resolveAssistantAvatar('file:///Users/demo/.biwork/assistant-avatars/custom-1.jpg')).toEqual({
      kind: 'fallback',
    });
  });
