...
const makeGitHubReleaseResponse = () => [
  {
    ...
    name: 'v1.9.22',
    body: 'release notes',
    html_url: 'https://github.com/knysfh/bibi_work/releases/tag/v1.9.22',
    published_at: '2026-04-29T00:00:00Z',
    prerelease: false,
    draft: false,
    assets: [
      {
        name: 'BiWork-1.9.22-mac-arm64.dmg',
        browser_download_url:
          'https://github.com/knysfh/bibi_work/releases/download/v1.9.22/BiWork-1.9.22-mac-arm64.dmg',
        size: 123,
        content_type: 'application/x-apple-diskimage',
      },
      {
        name: 'BiWork-1.9.22-win-x64.exe',
        browser_download_url: 'https://github.com/knysfh/bibi_work/releases/download/v1.9.22/BiWork-1.9.22-win-x64.exe',
        size: 456,
        content_type: 'application/vnd.microsoft.portable-executable',
      },
      {
        name: 'BiWork-1.9.22-linux-amd64.deb',
        browser_download_url:
          'https://github.com/knysfh/bibi_work/releases/download/v1.9.22/BiWork-1.9.22-linux-amd64.deb',
        size: 789,
      },
    ],
  },
...
describe('updateBridge CDN URL rewriting', () => {
  ...
  it('rewrites asset.url to the CDN path and keeps GitHub URL in fallbackUrl', async () => {
    ...
    try {
      ...
      const result = await handler({ repo: 'knysfh/bibi_work' });
      ...
      const macAsset = assets.find((a: { name: string }) => a.name === 'BiWork-1.9.22-mac-arm64.dmg');
      expect(macAsset).toBeDefined();
      expect(macAsset?.url).toBe('https://static.biwork.com/releases/1.9.22/BiWork-1.9.22-mac-arm64.dmg');
      expect(macAsset?.fallbackUrl).toBe(
        'https://github.com/knysfh/bibi_work/releases/download/v1.9.22/BiWork-1.9.22-mac-arm64.dmg'
      );

      const linuxAsset = assets.find((a: { name: string }) => a.name === 'BiWork-1.9.22-linux-amd64.deb');
      expect(linuxAsset?.url).toBe('https://static.biwork.com/releases/1.9.22/BiWork-1.9.22-linux-amd64.deb');
    } finally {
      vi.unstubAllGlobals();
    }
  });
  ...
  it('uses the normalized version (no v prefix) in the CDN path', async () => {
    ...
    try {
      ...
      expect(asset?.url).toMatch(/^https:\/\/static\.biwork\.com\/releases\/1\.9\.22\//);
...
describe('updateBridge allowlist includes CDN host', () => {
  it('accepts static.biwork.com URLs for download', async () => {
    vi.resetModules();
    vi.clearAllMocks();
    ...
    try {
      ...
      const result = await handler({
        ...
        url: 'https://static.biwork.com/releases/1.9.22/BiWork-1.9.22-mac-arm64.dmg',
        file_name: 'BiWork-1.9.22-mac-arm64.dmg',
      });
