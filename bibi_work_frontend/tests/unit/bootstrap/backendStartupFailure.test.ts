...
describe('classifyBackendStartupFailure', () => {
  it('classifies missing GLIBC symbols as an incompatible backend runtime', () => {
    ...
    error.details = {
      stage: 'early_exit',
      stderrTail:
        "/opt/BiWork/resources/bundled-bibi_work_backend/linux-x64/bibi_work_backend.bin: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.34' not found\n" +
        "/opt/BiWork/resources/bundled-bibi_work_backend/linux-x64/bibi_work_backend.bin: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.32' not found",
    };
  ...
  it('classifies missing startup directory preparation as a startup directory failure', () => {
    const error = new Error('bibi_work_backend startup directory preparation failed') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
      stage: 'spawn',
      workDir: 'D:\\ai\\BiWork\\工作目录',
      causeMessage: 'ENOENT: no such file or directory, mkdir D:\\ai\\BiWork\\工作目录',
    };
  ...
  it('classifies startup directory permission failures separately from incomplete installs', () => {
    ...
    error.details = {
      stage: 'spawn',
      workDir: 'D:\\ai\\BiWork\\工作目录',
      causeMessage: 'EPERM: operation not permitted, mkdir D:\\ai\\BiWork\\工作目录',
    };
  ...
  it('does not classify post-resolution binary spawn ENOENT as a startup directory failure', () => {
    const error = new Error('bibi_work_backend process emitted an error before startup') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
      stage: 'spawn_error',
      binaryPath: 'D:\\apps\\BiWork\\resources\\bundled-bibi_work_backend\\win32-x64\\bibi_work_backend.exe',
      causeMessage:
        'spawn D:\\apps\\BiWork\\resources\\bundled-bibi_work_backend\\win32-x64\\bibi_work_backend.exe ENOENT',
    };
  ...
  it('classifies recoverable database corruption boundary failures separately from data migration failures', () => {
    ...
    error.details = {
      ...
      stderrTail:
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.recoverable_corruption databasePath=/db/biwork-backend.db: failed to initialize application data',
    };
  ...
  it('classifies packaged macOS architecture mismatches separately from generic startup failures', () => {
    const error = new Error('BiWork package architecture does not match this Mac') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
