...
describe('httpBridge', () => {
  ...
  describe('error handling', () => {
    it('refreshes a rejected token and retries once with the replacement bearer token', async () => {
      setAccessToken('token-old');
      setAccessTokenProvider((forceRefresh) => (forceRefresh ? 'token-new' : peekAccessToken()));
      const fetchSpy = vi
        .fn()
      ...
      await expect(httpRequest<{ ok: boolean }>('GET', '/api/x')).resolves.toEqual({ ok: true });
      expect(fetchSpy).toHaveBeenCalledTimes(2);
      expect(fetchSpy.mock.calls[0][1]?.headers).toMatchObject({ Authorization: 'Bearer token-old' });
      expect(fetchSpy.mock.calls[1][1]?.headers).toMatchObject({ Authorization: 'Bearer token-new' });
    });
    ...
    it.each([
      {
        ...
        details: { legacy_code: 'MISSING_AUTH_HEADER' },
      },
      {
        status: 403,
        code: 'PERMISSION_DENIED',
        error: 'permission denied',
        details: { capability: 'agent.run.create' },
      },
      {
        status: 409,
        code: 'WORKSPACE_REVISION_CONFLICT',
        error: 'workspace file revision conflict',
      ...
      },
      {
        ...
        details: { component: 'biwork_compat' },
      },
    ])('parses Rust error envelope for HTTP $status', async ({ status, code, error, details }) => {
      const envelope = {
        success: false,
      ...
      try {
        ...
        expect(err.backendMessage).toBe(error);
  ...
  describe('httpRawRequest', () => {
    it('returns the raw response and preserves caller headers with bearer auth', async () => {
      setAccessToken('token-raw');
      const fetchSpy = vi.fn().mockResolvedValue(
        new Response('abcdef', {
          status: 206,
          headers: {
      ...
      const response = await httpRawRequest(
        'GET',
        '/api/v1/tool-result-artifacts/stream?tenant_id=t&object_reference_id=o',
        {
    ...
    it('throws BackendHttpError with structured body for raw request failures', async () => {
      const fetchSpy = vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ code: 'VALIDATION_ERROR', error: 'bad range', trace_id: 'trace-1' }), {
          status: 400,
          headers: { 'content-type': 'application/json' },
        })
      );
      ...
      await expect(httpRawRequest('GET', '/api/v1/tool-result-artifacts/stream')).rejects.toMatchObject({
        status: 400,
        code: 'VALIDATION_ERROR',
        backendMessage: 'bad range',
        traceId: 'trace-1',
      });
    });
  });
  ...
  describe('wsEmitter', () => {
    it('waits for a broker token before opening websocket subscriptions', async () => {
      vi.stubGlobal('window', { __backendPort: 13400 });
      vi.stubGlobal('WebSocket', FakeWebSocket as unknown as typeof WebSocket);
      vi.spyOn(console, 'debug').mockImplementation(() => {});
      FakeWebSocket.instances = [];

      const unsubscribe = wsEmitter('message.stream').on(() => {});

      expect(FakeWebSocket.instances).toHaveLength(0);

      setAccessToken('delayed-ws-token');
      ...
      expect(FakeWebSocket.instances[0].sentMessages).toEqual([
        JSON.stringify({ op: 'auth', access_token: 'delayed-ws-token' }),
        JSON.stringify({ op: 'subscribe', scope: 'conversation' }),
      ]);

      unsubscribe();
    });

    it('authenticates websocket opens and emits realtime.reconnected only after a prior open', async () => {
      globalThis.__biworkAccessToken = 'ws-bridge-token';
      vi.useFakeTimers();
      vi.stubGlobal('window', { __backendPort: 13400 });
      vi.stubGlobal('WebSocket', FakeWebSocket as unknown as typeof WebSocket);
      vi.spyOn(console, 'debug').mockImplementation(() => {});
      ...
      const unsubscribeMessageStream = wsEmitter('message.stream').on(() => {});
      ...
      expect(FakeWebSocket.instances[0].sentMessages).toEqual([
        JSON.stringify({ op: 'auth', access_token: 'ws-bridge-token' }),
        JSON.stringify({ op: 'subscribe', scope: 'conversation' }),
      ]);
    ...
    it('does not clear a replacement token when a stale socket is revoked', async () => {
      setAccessToken('stale-socket-token');
      vi.useFakeTimers();
      vi.stubGlobal('window', { __backendPort: 13400 });
      vi.stubGlobal('WebSocket', FakeWebSocket as unknown as typeof WebSocket);
      vi.spyOn(console, 'debug').mockImplementation(() => {});
      ...
      replacementSocket.dispatchOpen();
      await Promise.resolve();
      await Promise.resolve();

      staleSocket.dispatchMessage({ event: 'auth.revoked', payload: { reason: 'old session revoked' } });

      expect(peekAccessToken()).toBe('replacement-token');
    ...
    it('closes websocket and suppresses reconnect when the broker token is cleared', async () => {
      setAccessToken('ws-logout-token');
      vi.useFakeTimers();
      vi.stubGlobal('window', { __backendPort: 13400 });
      vi.stubGlobal('WebSocket', FakeWebSocket as unknown as typeof WebSocket);
      vi.spyOn(console, 'debug').mockImplementation(() => {});
      ...
      expect(socket.sentMessages).toEqual([
        JSON.stringify({ op: 'auth', access_token: 'ws-logout-token' }),
        JSON.stringify({ op: 'subscribe', scope: 'conversation' }),
      ]);
    ...
    it.each([
      { event: 'auth.revoked', payload: { reason: 'platform session has been revoked' } },
      { event: 'auth.failed', payload: { code: 'UNAUTHORIZED' } },
    ])('clears token and suppresses reconnect on $event', async ({ event, payload }) => {
      setAccessToken('ws-terminal-auth-token');
      vi.useFakeTimers();
      vi.stubGlobal('window', { __backendPort: 13400 });
      vi.stubGlobal('WebSocket', FakeWebSocket as unknown as typeof WebSocket);
      vi.spyOn(console, 'debug').mockImplementation(() => {});
      ...
      const events: unknown[] = [];
      const unsubscribe = wsEmitter(event).on((data: unknown) => events.push(data));
      const socket = FakeWebSocket.instances[0];
      socket.dispatchOpen();
      await Promise.resolve();
      await Promise.resolve();

      socket.dispatchMessage({ event, payload });
  ...
  describe('httpRequest', () => {
    ...
    it('adds the shared broker bearer token', async () => {
      globalThis.__biworkAccessToken = 'http-bridge-token';
      const fetchSpy = vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ data: { result: 'ok' } }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
      ...
      await httpRequest<{ result: string }>('GET', '/api/test');

      expect(fetchSpy).toHaveBeenCalledWith(expect.stringContaining('/api/test'), {
        method: 'GET',
        headers: { Authorization: 'Bearer http-bridge-token' },
        body: undefined,
      });
    });
