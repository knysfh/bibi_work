...
vi.mock('@/renderer/services/SkillImportService', () => ({
  uploadSkillZip: mocks.uploadSkillZip,
}));
...
describe('SkillsHubSettings', () => {
  ...
  const triggerManualImport = async () => {
    ...
    fireEvent.click(await screen.findByTestId('skill-import-local'));
  };
  ...
  it('uploads a ZIP package without requiring a shared desktop path', async () => {
    mocks.uploadSkillZip.mockResolvedValue({
      skill_name: 'uploaded-skill',
      skill_names: ['uploaded-skill'],
      failed: [],
    });
    ...
    const marker = await screen.findByTestId('btn-add-skill-manual');
    fireEvent.click((marker.closest('[role="menuitem"]') ?? marker) as HTMLElement);
    const input = await screen.findByTestId('skill-import-zip-upload');
    fireEvent.change(input, { target: { files: [new File(['zip'], 'skill.zip', { type: 'application/zip' })] } });

    await waitFor(() => expect(mocks.uploadSkillZip).toHaveBeenCalledWith(expect.any(File)));
