...
describe('selectRecentLogFiles', () => {
  it('returns every file from the N most recent non-empty days', () => {
    const files = [
      ...
      { path: '/a/2026-05-22.bibi_work_backend.log', mtime: Date.UTC(2026, 4, 22, 11), size: 200 },
...
describe('listLogFilesSync', () => {
  it('finds log files under dated year/month/day directories', () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'biwork-sentry-logs-'));
    try {
      const datedDir = path.join(logsDir, '2026', '07', '02');
      mkdirSync(datedDir, { recursive: true });
      writeFileSync(path.join(datedDir, '2026-07-02.bibi_work_backend.log'), 'backend\n');
      writeFileSync(path.join(datedDir, '2026-07-02.codex.log'), 'codex\n');
      ...
      expect(relative).toEqual([
        '2026-07-02.log',
        '2026/07/02/2026-07-02.bibi_work_backend.log',
        '2026/07/02/2026-07-02.codex.log',
      ]);
    } finally {
...
describe('captureBackendStartupFailure', () => {
  it('captures and flushes a dedicated backend startup failure with diagnostics', async () => {
    ...
    const error = new Error('bibi_work_backend failed to start within timeout') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
      stage: 'health_timeout',
      binaryPath: '/abs/path/bibi_work_backend',
      port: 33334,
    ...
    expect(scopeSetContext).toHaveBeenCalledWith('backend_startup', error.details);
  });
  ...
  it('sets bucketed health polling tags for backend startup timeouts', async () => {
    ...
    expect(scopeSetTag).toHaveBeenCalledWith('biwork.backend_startup.health_polling_delayed', 'true');
    expect(scopeSetTag).toHaveBeenCalledWith('biwork.backend_startup.health_attempts_bucket', '1');
    expect(scopeSetTag).toHaveBeenCalledWith('biwork.backend_startup.health_attempt_deficit_bucket', '76-150');
    expect(scopeSetTag).toHaveBeenCalledWith('biwork.backend_startup.health_timeout_overrun_bucket', 'over_60s');
    expect(scopeSetTag).toHaveBeenCalledWith('biwork.backend_startup.health_max_attempt_gap_bucket', '0ms');
  });

  it('sets backend data migration reason and boundary tags', async () => {
    scopeSetTag.mockClear();
    const error = new Error('bibi_work_backend exited before health check passed') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
      stage: 'early_exit',
      ...
      stderrTail:
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.migration databasePath=/db/biwork-backend.db: failed to initialize application data',
    };

    await captureBackendStartupFailure(error);

    expect(scopeSetTag).toHaveBeenCalledWith('biwork.backend_startup.reason', 'backend_data_migration_failed');
    expect(scopeSetTag).toHaveBeenCalledWith('biwork.backend_startup.boundary_code', 'BOOTSTRAP_DATA_INIT_FAILED');
    expect(scopeSetTag).toHaveBeenCalledWith('biwork.backend_startup.boundary_stage', 'database.migration');
  });
  ...
  it('sets local data repair reason and issue-kind tags', async () => {
    ...
    expect(scopeSetTag).toHaveBeenCalledWith('biwork.backend_startup.reason', 'backend_local_data_repair_failed');
    expect(scopeSetTag).toHaveBeenCalledWith(
      'biwork.backend_startup.local_data_issue_kind',
      'agent_metadata_invalid_utf8'
    );
    expect(scopeSetTag).toHaveBeenCalledWith('biwork.backend_startup.boundary_code', 'BOOTSTRAP_SERVICE_INIT_FAILED');
    expect(scopeSetTag).toHaveBeenCalledWith('biwork.backend_startup.boundary_stage', 'services.init');
  });
});
...
describe('initSentry beforeSend', () => {
  ...
  it('drops backend-port secondary errors after backend startup already failed', () => {
    ...
    const event = {
      exception: {
        values: [
          {
            value: '[WebUI] Cannot start: bibi_work_backend is not running (globalThis.__backendPort unset)',
          },
        ],
      },
    };
  ...
  it('keeps the primary backend startup failure even when its details contain secondary text', () => {
    ...
    const event = {
      tags: {
        'biwork.failure': 'backend_startup',
      },
      exception: {
        values: [
          {
  ...
  it('keeps user feedback reports even when diagnostics contain backend secondary text', () => {
    ...
    const event = {
      tags: {
        type: 'user-feedback',
        'biwork.installation_integrity.user_report': 'true',
      },
      extra: {
