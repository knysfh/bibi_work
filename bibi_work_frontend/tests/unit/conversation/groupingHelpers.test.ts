...
describe('buildGroupedHistory', () => {
  it('keeps scheduled-task conversations in the regular conversation timeline', () => {
    const result = buildGroupedHistory(
      [conversation('cron-conversation', { backend: 'bibi_work_backend', cron_job_id: 'job-1' }, 100)],
      t
    );
    ...
    const result = buildGroupedHistory(
      [
        conversation(
          ...
          {
            backend: 'bibi_work_backend',
            cron_job_id: 'job-1',
            workspace: '/repo/biwork',
  ...
  it('continues to hide team-owned conversations from the regular history', () => {
    const result = buildGroupedHistory(
      [conversation('team-conversation', { backend: 'bibi_work_backend', team_id: 'team-1' }, 100)],
      t
    );
