...
describe('agent error locale copy', () => {
  ...
  it('does not label app-side errors as direct BiWork ownership', () => {
    const localeDir = path.join(process.cwd(), 'packages/desktop/src/renderer/services/i18n/locales');
    ...
    for (const localeName of supportedLocaleNames) {
      ...
      expect(agentError.ownership.biwork, localeName).not.toMatch(/BiWork/);

      for (const [code, copy] of Object.entries<Record<string, string>>(agentError.codes)) {
        if (!code.startsWith('BIWORK_')) continue;

        expect(copy.title, `${localeName} ${code} title`).not.toMatch(/BiWork/);
        expect(copy.body, `${localeName} ${code} body`).not.toMatch(/BiWork/);
      }
    }
  });
