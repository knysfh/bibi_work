...
vi.mock('electron', () => ({
  ...
  app: {
    getPath: vi.fn(() => '/tmp/biwork-test-logs-nonexistent'),
...
describe('feedback logs', () => {
  it('collects top-level frontend logs and nested backend logs through the IPC handler', async () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'biwork-feedback-bridge-'));
    try {
  ...
  it('collects the same recent three log days used by user feedback reports', () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'biwork-feedback-logs-'));
    try {
      writeFileSync(path.join(logsDir, '2026-05-25.log'), 'today frontend\n');
      writeFileSync(path.join(logsDir, '2026-05-24.log'), 'yesterday frontend\n');
      ...
      expect(content).toContain('yesterday frontend');
  ...
  it('collects recent logs from dated year/month/day directories', () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'biwork-feedback-dated-logs-'));
    try {
      ...
      writeFileSync(path.join(previousDir, '2026-07-01.log'), 'yesterday frontend nested\n');
      ...
      expect(content).toContain('yesterday frontend nested');
      expect(content).toContain('third day frontend nested');
      expect(content).not.toContain('too old flat');
      expect(content).toContain('2026/07/02/2026-07-02.log');
    } finally {
      rmSync(logsDir, { recursive: true, force: true });
    }
  });
