...
describe('submitFeedbackReport', () => {
  ...
  it('submits a user-feedback event with tags, extra context, logs, and attachments', async () => {
    ...
    await submitFeedbackReport({
      ...
      collectLogs: true,
      description: '  BiWorkBackend   cannot start  ',
      extra: {
      ...
      moduleLabel: 'BiWork installation is incomplete',
      tags: {
        'biwork.installation_integrity.report_source': 'backend_startup_failure',
      },
    });
    ...
    expect(sentryMocks.setTag).toHaveBeenCalledWith(
      'biwork.installation_integrity.report_source',
      'backend_startup_failure'
    );
    expect(sentryMocks.captureEvent).toHaveBeenCalledWith(
      {
        level: 'info',
        message: 'BiWork installation is incomplete: BiWorkBackend cannot start',
        extra: {
          description: 'BiWorkBackend cannot start',
          installation_integrity: {
            source: 'backend_startup_failure',
