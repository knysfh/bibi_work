...
const makeAppDataDir = () => {
  const dir = mkdtempSync(path.join(tmpdir(), 'biwork-installer-last-failure-'));
  tempDirs.push(dir);
  return dir;
};
...
describe('installerLastFailure service', () => {
  it('consumes a valid installer-last-failure marker once from the AppData BiWork directory', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        blockers: [{ pid: 1234, name: 'BiWork.exe' }],
      })
    );
    ...
    expect(consumed?.blockers).toEqual([{ pid: 1234, name: 'BiWork.exe' }]);
