...
describe('AutoUpdaterService', () => {
  ...
  beforeEach(() => {
    ...
    appMock.getPath.mockImplementation(() => '/tmp/biwork-test');
    delete (autoUpdaterMock as { updateInfoAndProvider?: unknown }).updateInfoAndProvider;
    appMock.isPackaged = false;
    delete process.env.BIWORK_FORCE_DEV_AUTO_UPDATE;
    delete process.env.BIWORK_DEBUG_AUTO_UPDATE_CURRENT_VERSION;
  ...
  it('does not use the stable CDN updater when prerelease manual mode is enabled', async () => {
    autoUpdaterMock.checkForUpdates.mockResolvedValue({
      ...
      updateInfo: {
        version: '2.1.14',
        files: [{ url: 'BiWork-2.1.14-mac-arm64.dmg', sha512: 'sha512-value' }],
  ...
  it('configures electron-updater to read releases from the BiWork repository', async () => {
    const { autoUpdaterService } = await import('@/process/services/autoUpdaterService');
  ...
  it('enables forced updater checks in unpacked dev builds when requested', async () => {
    process.env.BIWORK_FORCE_DEV_AUTO_UPDATE = '1';
  ...
  it('overrides the updater current version only for forced unpacked dev checks', async () => {
    process.env.BIWORK_FORCE_DEV_AUTO_UPDATE = '1';
    process.env.BIWORK_DEBUG_AUTO_UPDATE_CURRENT_VERSION = '2.1.12';
  ...
  it('restores a completed cached auto-update when the downloaded package validates', async () => {
    const updateInfo = {
      version: '2.1.14',
      files: [{ url: 'BiWork-2.1.14-mac.zip', sha512: 'sha512-value' }],
      path: 'BiWork-2.1.14-mac.zip',
      sha512: 'sha512-value',
      releaseDate: '2026-06-08T00:00:00.000Z',
    };
    const fileInfo = {
      url: new URL('https://static.biwork.com/releases/2.1.14/BiWork-2.1.14-mac.zip'),
      info: { url: 'BiWork-2.1.14-mac.zip', sha512: 'sha512-value' },
    };
    const cachedUpdatePath = path.join('/cache/pending', 'BiWork-2.1.14-mac.zip');
  ...
  it('moves the process cwd to temp before the Windows updater handoff', async () => {
    setPlatform('win32');
    const tempRoot = path.join(process.env.TEMP || process.cwd(), `biwork-updater-cwd-test-${process.pid}`);
    const expectedCwd = path.join(tempRoot, 'biwork-updater-cwd');
    const chdir = vi.spyOn(process, 'chdir').mockImplementation(() => undefined);
    appMock.getPath.mockImplementation((name: string) => (name === 'temp' ? tempRoot : '/tmp/biwork-test'));
