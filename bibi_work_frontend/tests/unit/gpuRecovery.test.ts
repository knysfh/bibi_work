...
describe('gpuRecovery', () => {
  beforeEach(() => {
    ...
    crashListener = null;
    delete process.env.BIWORK_DISABLE_HARDWARE_ACCELERATION;
    vi.resetModules();
  });
  ...
  it('disables hardware acceleration when the environment flag is set', async () => {
    process.env.BIWORK_DISABLE_HARDWARE_ACCELERATION = '1';
    const { applyGpuRecoveryFlags } = await import('@/process/utils/gpuRecovery');
    applyGpuRecoveryFlags();
