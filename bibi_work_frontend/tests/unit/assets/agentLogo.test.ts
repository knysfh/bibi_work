...
const LOGOS: AgentLogoMap = {
  claude: 'https://assets.example.test/logos/ai-major/claude.svg',
  emojiagent: '🧠',
  gemini: 'https://assets.example.test/logos/ai-major/gemini.svg',
  opencode: 'https://assets.example.test/logos/tools/coding/opencode-light.svg',
  'openclaw-gateway': 'https://assets.example.test/logos/tools/openclaw.svg',
};
...
describe('agentLogo', () => {
  ...
  describe('resolveAgentLogo (backend lookup)', () => {
    it('returns logo path for known backend (case-insensitive)', () => {
      expect(resolveAgentLogo(LOGOS, { backend: 'Claude' })).toContain(
        'https://assets.example.test/logos/ai-major/claude.svg'
      );
    });

    it('returns logo for lowercase input', () => {
      expect(resolveAgentLogo(LOGOS, { backend: 'gemini' })).toContain(
        'https://assets.example.test/logos/ai-major/gemini.svg'
      );
    });
    ...
    it('does not expose local absolute paths as logo sources', () => {
      expect(resolveAgentLogo(LOGOS, { icon: '/Users/demo/.biwork/agent-avatars/custom.png' })).toBeNull();
    });

    it('does not expose legacy backend logo asset paths without an owner', () => {
      expect(resolveAgentLogo({ gemini: '/api/assets/logos/ai-major/gemini.svg' }, { backend: 'gemini' })).toBeNull();
      expect(
        resolveAgentLogo(
          { gemini: 'http://127.0.0.1:8361/api/assets/logos/ai-major/gemini.svg' },
          { backend: 'gemini' }
        )
      ).toBeNull();
    });
  ...
  describe('resolveAgentAvatar', () => {
    ...
    it('falls back to backend catalog logos when explicit local paths leak through', () => {
      expect(
        resolveAgentAvatar(LOGOS, { icon: '/Users/demo/.biwork/agent-avatars/custom.png', backend: 'claude' })
      ).toEqual({
        kind: 'image',
        value: 'https://assets.example.test/logos/ai-major/claude.svg',
      });
    });
      ...
      bridgeMocks.getManagedAgents.mockResolvedValue([
        {
          ...
          installed: true,
          status: 'online',
          icon: 'https://assets.example.test/logos/ai-major/claude.svg',
        },
      ]);

      await expect(fetchAgentLogos()).resolves.toEqual({
        acp: 'https://assets.example.test/logos/ai-major/claude.svg',
        'agent-claude': 'https://assets.example.test/logos/ai-major/claude.svg',
        claude: 'https://assets.example.test/logos/ai-major/claude.svg',
      });
