...
describe('release packaging configuration', () => {
  ...
  it('uploads mac zip artifacts without a stale Windows zip glob', () => {
    ...
    expect(workflow).toContain('out/BiWork-*-mac-*.zip');
    expect(workflow).not.toContain('out/BiWork-*-win32-*.zip');
  });
  ...
  itWithBash('fails release asset preparation when a mac zip is missing', () => {
    const tempDir = mkdtempSync(resolve(tmpdir(), 'biwork-release-assets-'));
    ...
    try {
      ...
      rmSync(resolve(artifactsDir, 'macos-build-arm64', 'BiWork-1.0.0-mac-arm64.zip'), { force: true });
