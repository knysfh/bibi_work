 ...
 */
const REMOVED_AUTO_INJECT_NAME = 'biwork-skills';
const REMOVED_AUTO_INJECT_DIR_NAME = 'biwork-skills';
...
function resolveBackendBinary(): string {
  const candidates = [
    process.env.BIWORK_BACKEND_BINARY,
    path.join(os.homedir(), '.cargo', 'bin', 'bibi_work_backend'),
  ].filter((x): x is string => typeof x === 'string' && x.length > 0);
  for (const c of candidates) {
    if (fs.existsSync(c)) return c;
  }
  throw new Error(
    'bibi_work_backend binary not found. Set BIWORK_BACKEND_BINARY or install to ~/.cargo/bin/bibi_work_backend.'
  );
}
...
test.describe('Built-in Skill Migration (T3)', () => {
  ...
  test('S7: builtin skills in /api/skills expose an absolute, readable location for export', async ({ page }) => {
    ...
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'biwork-e2e-s7-export-'));
    try {
  ...
  //
  // Run against a sibling `bibi_work_backend` process on port 25903 against a
  ...
  test.describe('Cold-start invariants (sibling backend)', () => {
    ...
    async function startBackend(): Promise<void> {
      ...
      const logPath = path.join(dataDir, 'sibling-bibi_work_backend.log');
      const logFd = fs.openSync(logPath, 'a');
      const parentEnv = { ...process.env };
      // Scrub any env vars that would leak main-Electron backend state.
      delete parentEnv.BIWORK_EXTENSIONS_PATH;
      delete parentEnv.BIWORK_EXTENSION_STATES_FILE;
      delete parentEnv.BIWORK_E2E_TEST;
      delete parentEnv.BIWORK_CDP_PORT;
      delete parentEnv.BIWORK_BUILTIN_SKILLS_PATH;
    ...
    test.beforeEach(() => {
      dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'biwork-e2e-builtin-skill-'));
    });
    ...
    test('S8: legacy {cacheDir}/builtin-skills/ is gone after the current Electron boot', async () => {
      ...
      // `~/.biwork-config` tree. Failing that we at least assert the
      ...
      const candidates = [
        path.join(os.homedir(), '.biwork-config', 'builtin-skills'),
        path.join(os.homedir(), '.biwork-config-dev', 'builtin-skills'),
      ];
      ...
      test.info().annotations.push({
        ...
        description:
          survivors.length === 0
            ? 'no legacy builtin-skills cache dirs detected under ~/.biwork-config*'
