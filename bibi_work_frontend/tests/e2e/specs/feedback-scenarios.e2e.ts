...
async function closeFeedbackModal(page: Page) {
  ...
  // button class — scoped to avoid matching the Agent editor's BiWorkModal
  // close button (which uses aria-label='Close' instead).
  await page.locator('.biwork-modal-close-btn').first().click();
  await expect(page.locator(MODAL_BODY)).toBeHidden({ timeout: 5_000 });
}

/** Close any open BiWorkModal (e.g. the Agent editor) so the next test starts clean. */
...
async function openCustomAgentEditor(page: Page, command: string) {
  // Defensive: close any BiWorkModal left over from a prior test so the
...
test('[5] Agent fail_cli alert surfaces feedback pill (module=agent-detection)', async ({ page }) => {
  await openCustomAgentEditor(page, 'biwork-e2e-missing-binary-xyz');
