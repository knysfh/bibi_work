...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'biwork-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'biwork-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      BIWORK_EXTENSIONS_PATH: emptyExtensionsDir,
      BIWORK_EXTENSION_STATES_FILE: extensionStatesFile,
