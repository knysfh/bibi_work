...
test.describe('Installation integrity failure dialog', () => {
  test('shows diagnostics actions and records a user report', async () => {
    ...
    const electronApp = await electron.launch({
      ...
      env: {
        ...process.env,
        BIWORK_DEBUG_BACKEND_STARTUP_FAILURE: 'backend_incomplete_installation',
    ...
    try {
      ...
      await expect(page.getByTestId('installation-integrity-description')).toContainText(/BiWork/);
