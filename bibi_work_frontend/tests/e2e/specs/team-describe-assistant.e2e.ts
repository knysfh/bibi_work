...
/** Backend /api/teams/:id GET response shape — aligns with bibi_work_backend schema. */
  ...
  test('describes a preset and then spawns it as a teammate via the real TCP bridge', async ({ page }) => {
    ...
    try {
      ...
      try {
        ...
        expect(describeText).toContain('Backend: codex');
        ...
        expect(spawned!.assistant_backend ?? spawned!.backend).toBe('codex'); // preset backend wins
      } finally {
        client.close();
      }
    } finally {
