...
// Read the real BiWork version from the repo-root package.json.
  ...
  return {
    main: {
      plugins: [
        ...
        // '@biwork/web-host' excluded so its TS sources (which use ESM ".js" import specifiers)
        // are bundled by esbuild rather than left as `require('@biwork/web-host')`, which Node
        // cannot resolve because the package ships no compiled .js files (workspace-only).
        externalizeDepsPlugin({ exclude: ['fix-path', '@biwork/web-host'] }),
        ...(isDevelopment
          ? [
              {
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another BiWork clone is running),
      ...
      define: {
        'process.env.NODE_ENV': JSON.stringify(mode),
        'process.env.env': JSON.stringify(process.env.env),
        'process.env.BIWORK_MULTI_INSTANCE': JSON.stringify(process.env.BIWORK_MULTI_INSTANCE ?? ''),
        'process.env.SENTRY_DSN': JSON.stringify(process.env.SENTRY_DSN ?? ''),
        // Inject the real BiWork version (root package.json) so renderer code
