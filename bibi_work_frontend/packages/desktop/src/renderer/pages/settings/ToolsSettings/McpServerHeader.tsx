...
const getStatusPopoverContent = (
  ...
  const checkedAt = formatStatusTimestamp(server.last_health_check || server.updated_at);
    ...
    <div className='max-w-300px space-y-2 text-13px leading-20px'>
      ...
      {(server.consecutive_failures || 0) > 1 ? (
        <div className='text-12px leading-18px text-t-secondary'>
          {t?.('settings.mcpConsecutiveFailures', {
            count: server.consecutive_failures,
            defaultValue: '{{count}} consecutive failed checks',
          })}
        </div>
      ) : null}
      {checkedAt ? (
      ...
      {!isReadOnly && (
        <div className='flex items-center gap-2 invisible group-hover:visible' onClick={(e) => e.stopPropagation()}>
          <Tooltip
            content={
              server.enabled
                ? t('settings.mcpDisableServer', { defaultValue: 'Disable MCP server' })
                : t('settings.mcpEnableServer', { defaultValue: 'Enable MCP server' })
            }
          >
            <Switch
              size='small'
              ...
              aria-label={
                server.enabled
                  ? t('settings.mcpDisableServerNamed', {
                      name: server.name,
                      defaultValue: 'Disable {{name}}',
                    })
                  : t('settings.mcpEnableServerNamed', {
                      name: server.name,
