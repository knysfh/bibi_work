...
const UPSTREAM_AGENT_HUB_REPO_URL = 'https://github.com/iOfficeAI/AionHub';
...
export const AgentHubModal: React.FC<AgentHubModalProps> = ({ visible, onCancel }) => {
  ...
  const modalWidth = agents.length === 0 ? 520 : Math.min(1000, Math.max(360, agents.length * 230 + 48));
  const actionButtonClassName = '!min-w-80px !rounded-9px !px-10px';
  const openUpstreamHubRepo = () => {
    void openExternalUrl(UPSTREAM_AGENT_HUB_REPO_URL).catch(console.error);
  };
    ...
    >
      <div className='max-h-[70vh] overflow-y-auto pr-4px'>
        <div className='mb-12px flex flex-wrap items-center justify-start gap-x-6px gap-y-2px text-left'>
          ...
          <Link className='text-12px leading-18px' onClick={openUpstreamHubRepo}>
            {t('settings.agentManagement.marketContributionAction', {
              defaultValue: 'Open a PR for BiWork Hub',
            })}
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div
            ...
            className='grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] justify-items-center gap-10px'
          >
            {agents.map((agent) => {
              ...
              return (
                <div
                  ...
                  className='flex min-h-[144px] w-full max-w-[220px] flex-col rounded-12px border border-solid border-[var(--color-border-2)] bg-[var(--color-bg-2)] p-10px transition-colors hover:border-[var(--color-border-3)]'
                >
                  <Typography.Text
                    bold
