...
import { uploadSkillZip, type SkillImportResult } from '@/renderer/services/SkillImportService';
...
const getImportHistoryStatusClass = (group: SkillImportHistoryGroup) => {
  if (group.failedCount > 0 || group.records.some((record) => record.status === 'overwritten')) {
...
const SkillsHubSettings: React.FC<SkillsHubSettingsProps> = ({ withWrapper = true }) => {
  ...
  const reportImportResult = (result: SkillImportResult) => {
    ...
    if (notice.importedNames.length > 0) setSearchQuery('');
    void fetchData();
  };
  ...
  const handleRemoteImport = async () => {
    const source = remoteSkillUrl.trim();
    if (!source) return;
    setImporting(true);
    try {
  ...
  const handleZipUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImporting(true);
    try {
      reportImportResult(await uploadSkillZip(file));
      setImportModalVisible(false);
    } catch (error) {
  ...
  const handleManualImport = () => setImportModalVisible(true);
      ...
      <div data-testid='official-skills-section'>
        <p className='m-0 mb-12px text-12px leading-relaxed text-t-tertiary'>
          {t('settings.skillsHub.officialHint', {
            defaultValue: 'Built-in skills maintained by BiWork — read-only and updated with each release.',
          })}
        </p>
  ...
  const page = withWrapper ? <SettingsPageWrapper>{mainContent}</SettingsPageWrapper> : mainContent;
  return (
    <>
      {page}
      <Modal
        visible={importModalVisible}
        title={t('settings.skillsHub.importDialogTitle', { defaultValue: 'Import skill' })}
        footer={null}
        style={{ width: 560 }}
        unmountOnExit
        onCancel={() => !importing && setImportModalVisible(false)}
      >
        <div className='flex flex-col gap-16px' data-testid='skill-import-dialog'>
          <section className='flex flex-col gap-8px rounded-8px border border-solid border-[var(--color-border-2)] p-14px'>
            <div className='font-medium text-t-primary'>
              {t('settings.skillsHub.importFromUrl', { defaultValue: 'Import from URL' })}
            </div>
            <div className='text-12px text-t-secondary'>
              {t('settings.skillsHub.importUrlHelp', {
                defaultValue: 'Use a GitHub repository URL or any HTTPS URL returning SKILL.md or a ZIP archive.',
              })}
            </div>
            <div className='flex gap-8px'>
              <Input
                ...
                onChange={setRemoteSkillUrl}
                onPressEnter={() => void handleRemoteImport()}
              />
              <Button
                data-testid='skill-import-url-submit'
                type='primary'
                loading={importing}
                disabled={!remoteSkillUrl.trim()}
                onClick={() => void handleRemoteImport()}
              >
                {t('settings.skillsHub.importAction', { defaultValue: 'Import' })}
              </Button>
            </div>
          </section>
          <section className='flex flex-col gap-8px rounded-8px border border-solid border-[var(--color-border-2)] p-14px'>
            <div className='font-medium text-t-primary'>
              {t('settings.skillsHub.importLocalPackage', { defaultValue: 'Import local package' })}
            </div>
            <div className='flex flex-wrap gap-8px'>
              <Button data-testid='skill-import-local' disabled={importing} onClick={() => void handleLocalImport()}>
                {t('settings.skillsHub.chooseFolderOrZip', { defaultValue: 'Choose folder or ZIP' })}
              </Button>
              <Button
              ...
              >
                {t('settings.skillsHub.uploadZip', { defaultValue: 'Upload ZIP' })}
              </Button>
              <input
                ref={zipUploadInputRef}
                data-testid='skill-import-zip-upload'
                className='hidden'
                type='file'
                accept='.zip,application/zip'
                disabled={importing}
                onChange={(event) => void handleZipUpload(event)}
              />
            </div>
            <div className='text-12px text-t-secondary'>
              {t('settings.skillsHub.uploadZipHelp', {
                defaultValue: 'ZIP upload works in both the desktop app and WebUI. Packages must contain SKILL.md.',
              })}
            </div>
          </section>
        </div>
