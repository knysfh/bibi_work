...
const SettingsSider: React.FC<{ collapsed?: boolean; tooltipEnabled?: boolean }> = ({
  ...
  const { menus, groupHeaderAt } = useMemo(() => {
    ...
    const aboutIndex = result.findIndex((item) => item.id === 'about');
    result.splice(aboutIndex >= 0 ? aboutIndex : result.length, 0, {
      id: 'logout',
      label: t('common.logout'),
      icon: <CloseOne />,
      action: handleLogout,
      testId: 'desktop-logout',
    });
  ...
  }, [t, isDesktop, extensionTabs, resolveExtTabName, handleLogout]);
  ...
  return (
    ...
    >
      {menus.map((item, index) => {
        const isSelected = item.path ? pathname.includes(item.path) : false;
