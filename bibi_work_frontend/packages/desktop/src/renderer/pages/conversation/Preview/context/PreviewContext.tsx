...
export interface PreviewMetadata {
  ...
  revision?: number; // Rust-backed workspace/artifact revision for stale-write checks
...
const PREVIEW_TABS_KEY = 'biwork_preview_tabs';
const PREVIEW_ACTIVE_TAB_ID_KEY = 'biwork_preview_active_tab_id';
const LEGACY_PREVIEW_STATE_KEY = 'biwork_preview_state';
...
const validRevision = (value: unknown): number | undefined => {
    ...
    async (tabId?: string) => {
      ...
      if (tab.metadata?.file_path && tab.metadata?.workspace) {
        try {
          ...
          const expected_revision = validRevision(tab.metadata.revision);
          ...
          if (success) {
            let savedRevision = expected_revision !== undefined ? expected_revision + 1 : undefined;
            try {
              const latestMetadata = await ipcBridge.fs.getFileMetadata.invoke({
                path: file_path,
                workspace: tab.metadata.workspace,
              });
              savedRevision = validRevision(latestMetadata?.revision) ?? savedRevision;
            } catch (error) {
              console.warn('[PreviewContext] Failed to refresh file metadata after save:', file_path, error);
            }
              ...
              prevTabs.map((t) => {
                if (t.id === targetTabId) {
                  return {
                    ...
                    metadata: savedRevision !== undefined ? { ...t.metadata, revision: savedRevision } : t.metadata,
                  };
                }
                return t;
              })
  ...
  const checkFileUpdate = useCallback(
    (tab: PreviewTab) => {
      ...
      void ipcBridge.fs.getFileMetadata
        ...
        .then((metadata) => {
          if (!metadata) return;
          const nextRevision = validRevision(metadata.revision);
              ...
              setTabs((latest) =>
                latest.map((t) => {
                  ...
                  return {
                    ...
                    originalContent: content,
                    isDirty: false,
                    metadata: nextRevision !== undefined ? { ...t.metadata, revision: nextRevision } : t.metadata,
                  };
                })
              );
            })
