...
export const isAlwaysAllowConfirmationValue = (value: string): boolean =>
  ['proceed_always', 'proceed_always_server', 'proceed_always_tool', 'allow_always'].includes(value);
...
const MessagePermission: React.FC<MessagePermissionProps> = React.memo(({ message }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const always_allow = isAlwaysAllowConfirmationValue(selected);
  ...
  return (
    <Card
      className='mb-4 w-full min-w-0 overflow-hidden'
      bordered={false}
    ...
    >
      <div className='min-w-0 space-y-4'>
        <div className='flex min-w-0 items-center space-x-2'>
          ...
          <Text className='block min-w-0 break-words'>{displayTitle}</Text>
        </div>
        {command_type && (
          <div>
        ...
        {description && description !== displayTitle && (
          <div>
            <Text className='text-xs text-t-secondary break-words'>{description}</Text>
          </div>
        )}
        {!hasResponded && (
          <>
