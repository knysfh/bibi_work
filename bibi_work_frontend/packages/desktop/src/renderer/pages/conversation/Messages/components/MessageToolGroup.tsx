...
import { Copy, Download, Earth, LoadingOne } from '@icon-park/react';
...
type BrowserToolResult = Extract<
  NonNullable<IMessageToolGroupProps['message']['content'][number]['result_display']>,
  { kind: 'browser' }
>;

const BrowserToolCard: React.FC<{
  name: string;
  status: IMessageToolGroupProps['message']['content'][number]['status'];
  description: string;
  result?: BrowserToolResult;
}> = ({ name, status, description, result }) => {
  const isRunning = !['Success', 'Error', 'Canceled'].includes(status);
  const statusColor =
    status === 'Error'
      ? 'rgb(var(--danger-6))'
      : status === 'Canceled'
  ...
  return (
    <div
      className='w-full min-w-0 overflow-hidden rd-8px border border-solid border-[var(--color-border-2)] bg-[var(--bg-1)] px-10px py-8px'
      data-testid='browser-tool-card'
    >
      <div className='flex min-w-0 items-center gap-8px'>
        ...
        <div className='min-w-0 flex-1'>
          <div className='flex min-w-0 flex-wrap items-center gap-6px'>
            <span className='text-13px font-500 text-t-primary break-words'>{name}</span>
            <span className='flex items-center gap-4px text-11px text-t-secondary'>
              <span className='h-6px w-6px rd-full' style={{ backgroundColor: statusColor }} />
              {isRunning ? 'Running' : status}
            </span>
          </div>
          {(result?.title || description) && (
            <div className='mt-3px min-w-0 text-12px text-t-secondary break-words'>{result?.title || description}</div>
          )}
          {result?.url && <div className='mt-3px min-w-0 text-11px text-t-secondary break-all'>{result.url}</div>}
        </div>
      </div>
      {result?.action && (
        <div className='mt-6px text-11px text-t-secondary'>
          Action: <span className='text-t-primary'>{result.action}</span>
          {typeof result.element_count === 'number' ? ` · ${result.element_count} interactive elements` : ''}
        </div>
      )}
      {result?.text && (
        <div className='mt-6px min-w-0'>
          <CollapsibleContent maxHeight={120} defaultCollapsed={true} useMask={false}>
            <div className='whitespace-pre-wrap break-words text-11px text-t-secondary'>{result.text}</div>
          </CollapsibleContent>
        </div>
      )}
    </div>
      ...
      {message.content.map((content, index) => {
        ...
        if (name.startsWith('browser_')) {
          const browserResult =
            ...
            'kind' in result_display &&
            result_display.kind === 'browser'
              ? (result_display as BrowserToolResult)
              : undefined;
          return (
            <div className='w-full min-w-0' key={call_id}>
              <BrowserToolCard name={name} status={status} description={description} result={browserResult} />
            </div>
          );
        }
