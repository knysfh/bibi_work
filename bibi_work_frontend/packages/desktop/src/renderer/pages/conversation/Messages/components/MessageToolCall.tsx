...
const MessageToolCall: React.FC<{ message: IMessageToolCall }> = ({ message }) => {
  ...
  const hasDetail = normalized.input || normalized.output || normalized.inputFields?.length || normalized.resultSummary;
        ...
        <div className='tool-detail-panel m-l-20px m-t-4px'>
          ...
          {normalized.resultSummary && (
            <div className='tool-detail-section'>
              ...
              <div className='tool-display-result'>{normalized.resultSummary}</div>
            </div>
          )}
