...
const CreateTaskDialog: React.FC<CreateTaskDialogProps> = ({
  ...
  const handleAcpModelSelect = useCallback((model: React.SetStateAction<string | null>) => {
    setModelId((current) => {
      const next = typeof model === 'function' ? model(current ?? null) : model;
  ...
  const showModelSelector = Boolean(resolvedBackend && acpCachedModelInfo);
  ...
  const handleSubmit = async () => {
    try {
      ...
      try {
        ...
        console.error('[CreateTaskDialog] Failed to refresh scheduled tasks after save:', error);
      }
      onClose();
    } catch (err) {
