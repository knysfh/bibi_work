...
import { setAccessTokenProvider, setAuthSessionInvalidator } from '@/common/auth/authTokenBroker';
if (window.electronAPI?.getAuthAccessToken) {
  setAccessTokenProvider((forceRefresh) => window.electronAPI?.getAuthAccessToken?.(forceRefresh));
  setAuthSessionInvalidator(() => window.electronAPI?.invalidateDesktopAuth?.());
}
  ...
  void import('@sentry/electron/renderer')
    .then((Sentry) => {
      Sentry.withScope((scope) => {
        scope.setTag('biwork.installation_integrity', event.failure_kind ?? 'unknown');
        scope.setTag('biwork.runtime_resource', event.resource);
        scope.setTag('biwork.runtime_resource_id', event.resource_id ?? '');
        scope.setTag('biwork.runtime_scope', event.scope.kind);
