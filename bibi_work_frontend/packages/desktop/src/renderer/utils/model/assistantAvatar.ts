...
const LEGACY_BACKEND_LOGO_ASSET_PATH = '/api/assets/logos';

function isLegacyBackendLogoAssetPathname(pathname: string): boolean {
  return pathname === LEGACY_BACKEND_LOGO_ASSET_PATH || pathname.startsWith(`${LEGACY_BACKEND_LOGO_ASSET_PATH}/`);
}

export function isLegacyBackendLogoAssetPath(value: string): boolean {
  const trimmed = value.trim();
  if (isLegacyBackendLogoAssetPathname(trimmed)) return true;

  try {
    return isLegacyBackendLogoAssetPathname(new URL(trimmed).pathname);
  } catch {
    return false;
  }
}
...
export function isBackendRelativeAssetPath(value: string): boolean {
  return (value.startsWith('/api/') || value.startsWith('/assets/')) && !isLegacyBackendLogoAssetPath(value);
}
...
export function resolveAssistantAvatar(avatar: string | undefined): AssistantAvatar {
  ...
  if (isLegacyBackendLogoAssetPath(value)) {
    return { kind: 'fallback' };
  }
