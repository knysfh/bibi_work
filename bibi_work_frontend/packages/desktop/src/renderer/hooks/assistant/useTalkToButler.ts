...
/** Backend manifest id of the built-in BiWork Butler assistant. */
const BUTLER_ASSISTANT_ID = 'biwork-assistant';
 ...
 * select the BiWork Butler, and pre-fill the chat input with a ready-made
      ...
      try {
        ...
        if (butler) {
          ...
          if (butler.enabled === false) {
            ...
            Message.success(
              t('settings.talkToButler.enabledToast', { defaultValue: 'Enabled the BiWork Butler for you' })
            );
          }
        }
      } catch (error) {
