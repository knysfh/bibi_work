 ...
 */
export interface BiWorkCollapseProps {
 ...
 */
export interface BiWorkCollapseItemProps {
 ...
 */
const BiWorkCollapseItem: React.FC<BiWorkCollapseItemProps> = ({ children }) => <>{children}</>;
BiWorkCollapseItem.displayName = 'BiWorkCollapseItem';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <BiWorkCollapse defaultActiveKey={['1']}>
 *   <BiWorkCollapse.Item name="1" header="面板1">
 *     内容1
 *   </BiWorkCollapse.Item>
 *   <BiWorkCollapse.Item name="2" header="面板2">
 *     内容2
 *   </BiWorkCollapse.Item>
 * </BiWorkCollapse>
 *
 * // 手风琴模式 / Accordion mode
 * <BiWorkCollapse accordion defaultActiveKey="1">
 *   <BiWorkCollapse.Item name="1" header="面板1">内容1</BiWorkCollapse.Item>
 *   <BiWorkCollapse.Item name="2" header="面板2">内容2</BiWorkCollapse.Item>
 * </BiWorkCollapse>
 *
 * // 自定义图标 / Custom icon
 * <BiWorkCollapse
 ...
 * ```
 */
const BiWorkCollapseComponent: React.FC<BiWorkCollapseProps> & { Item: typeof BiWorkCollapseItem } = ({
  children,
  className,
  defaultActiveKey,
  activeKey,
  ...
  const items = useMemo(() => {
    return React.Children.toArray(children).filter((child): child is React.ReactElement<BiWorkCollapseItemProps> => {
      return React.isValidElement(child) && child.type === BiWorkCollapseItem;
    });
  }, [children]);
...
BiWorkCollapseComponent.Item = BiWorkCollapseItem;

export default BiWorkCollapseComponent;
