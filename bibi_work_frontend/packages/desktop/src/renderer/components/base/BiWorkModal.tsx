...
/** BiWorkModal 组件 Props */
export interface BiWorkModalProps extends Omit<ModalProps, 'title' | 'footer'> {
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <BiWorkModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </BiWorkModal>
 *
 * // 预设尺寸 / Preset size
 * <BiWorkModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </BiWorkModal>
 *
...
const BiWorkModal: React.FC<BiWorkModalProps> = ({
  children,
  size,
  header,
  footer,
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={onCancel}
      className={`biwork-modal ${className}`}
      style={finalStyle}
      getPopupContainer={() => document.body}
    >
      <div className='biwork-modal-wrapper' style={{ borderRadius: borderRadiusVal }}>
        {renderHeader()}
        <div className='biwork-modal-body-content' style={bodyInlineStyle}>
          {children}
        </div>
        {renderFooter()}
      </div>
...
BiWorkModal.displayName = 'BiWorkModal';

export default BiWorkModal;
