...
export const UPDATE_AVAILABLE_EVENT = 'biwork-update-available';
...
const RELEASES_PAGE_URL = 'https://github.com/knysfh/bibi_work/releases';
...
export const useUpdateNotificationController = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('biwork-open-update-modal', handleWindowOpen);
    ...
    return () => {
      removeOpenListener();
      window.removeEventListener('biwork-open-update-modal', handleWindowOpen);
