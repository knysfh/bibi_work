...
export const SubModal: React.FC<SubModalProps> = ({ visible, onCancel, title, children }) => {
  return (
    <BiWorkModal
      ...
      title={title}
    >
      <BiWorkScrollArea className='h-full px-20px pb-16px text-14px text-t-primary'>{children}</BiWorkScrollArea>
    </BiWorkModal>
  );
};
...
const SettingsModal: React.FC<SettingsModalProps> = ({ visible, onCancel, defaultTab = 'model' }) => {
  ...
  const desktopMenu = (
    <BiWorkScrollArea className='flex-shrink-0 b-color-border-2 scrollbar-hide' style={{ width: `${SIDEBAR_WIDTH}px` }}>
