...
const ChannelModalContent: React.FC = () => {
  ...
  const applyPluginStatus = useCallback((status: IChannelPluginStatus) => {
  ...
  const handleTogglePlugin = async (enabled: boolean) => {
    ...
    try {
      if (enabled) {
        ...
        const status = await channel.enablePlugin.invoke({
          plugin_id: 'telegram',
        ...
        const status = await channel.disablePlugin.invoke({
          plugin_id: 'telegram',
        });
        applyPluginStatus(status);
  ...
  const handleToggleExtensionPlugin = useCallback(
    async (pluginType: string, enabled: boolean) => {
      ...
      try {
        if (enabled) {
          ...
          const nextStatus = await channel.enablePlugin.invoke({
            plugin_id: status.id || pluginType,
            config: fieldValues,
          });
          applyPluginStatus(nextStatus);
          ...
          const nextStatus = await channel.disablePlugin.invoke({
            plugin_id: status.id || pluginType,
          });
          applyPluginStatus(nextStatus);
          Message.success(
    ...
    [extensionStatuses, extensionFieldValues, t, applyPluginStatus]
  );
  ...
  const channels: ChannelConfig[] = useMemo(() => {
    const telegramChannel: ChannelConfig = {
      ...
      description: t('settings.channels.telegramDesc', 'Chat with BiWork assistant via Telegram'),
      status: 'active',
    ...
    const larkChannel: ChannelConfig = {
      ...
      description: t('settings.channels.larkDesc', 'Chat with BiWork assistant via Lark or Feishu'),
      status: 'active',
    ...
    const dingtalkChannel: ChannelConfig = {
      ...
      description: t('settings.channels.dingtalkDesc', 'Chat with BiWork assistant via DingTalk'),
      status: 'active',
    ...
    const weixinChannel: ChannelConfig = {
      ...
      description: t('settings.channels.weixinDesc', 'Chat with BiWork assistant via WeChat'),
      status: 'active',
    ...
    const wecomChannel: ChannelConfig = {
      ...
      description: t('settings.channels.wecomDesc', 'Chat with BiWork assistant via WeCom (Enterprise WeChat)'),
    ...
    const comingSoonChannels: ChannelConfig[] = [
      {
        ...
        description: t('settings.channels.slackDesc', 'Chat with BiWork assistant via Slack'),
        status: 'coming_soon' as const,
        enabled: false,
        disabled: true,
        content: (
      ...
      },
      {
        ...
        description: t('settings.channels.discordDesc', 'Chat with BiWork assistant via Discord'),
        status: 'coming_soon' as const,
        enabled: false,
        disabled: true,
        content: (
  ...
  const channelGuideText = t('settings.webui.featureChannelsDesc', {
    defaultValue: 'Connect Telegram, Lark, and DingTalk to interact with BiWork from IM apps.',
  });
  ...
  return (
    <BiWorkScrollArea className={isPageMode ? 'h-full' : ''}>
