...
import CredentialRotationStatus from './CredentialRotationStatus';
...
const ModelModalContent: React.FC = () => {
  ...
  const performHealthCheck = async (platform: IProvider, modelName: string) => {
    ...
    try {
      const result = await ipcBridge.mode.testProvider.invoke({
  ...
  return (
    ...
    >
      ...
      {/* Content Area */}
      <BiWorkScrollArea className='flex-1 min-h-0' disableOverflow={isPageMode}>
        {!data || data.length === 0 ? (
          <div className='flex flex-col items-center justify-center py-40px'>
            ...
            <p className='text-14px text-t-secondary text-center max-w-400px'>
              ...
              <a
                href='https://github.com/knysfh/bibi_work/wiki/LLM-Configuration'
                target='_blank'
