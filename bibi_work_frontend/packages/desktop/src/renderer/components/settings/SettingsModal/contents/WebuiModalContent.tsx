...
const WebuiModalContent: React.FC = () => {
    ...
    const unsubscribe = webui.statusChanged.on((data) => {
      ...
      if (data.running) {
        setStatus((prev) => ({
          ...
          lanIP: prev?.lanIP,
          authMode: data.authMode ?? prev?.authMode,
    ...
    try {
      if (enabled) {
        ...
        setStatus((prev) => ({
          ...
          lanIP: responseIP,
          authMode: startResult.authMode ?? prev?.authMode,
  ...
  const supportsLocalWebuiCredentials = status?.authMode !== 'ferriskey_oidc';

  // 打开设置新密码弹窗 / Open set new password modal
  const handleResetPassword = () => {
    if (!supportsLocalWebuiCredentials) return;
    form.resetFields();
  ...
  const generateQRCode = useCallback(async () => {
    if (!status?.running || !supportsLocalWebuiCredentials) return;
  ...
  useEffect(() => {
    if (status?.running && status.allowRemote && supportsLocalWebuiCredentials && !qrUrl) {
      void generateQRCode();
    }
    // 清理定时器 / Cleanup timer
    return () => {
  ...
  }, [status?.allowRemote, status?.running, supportsLocalWebuiCredentials, generateQRCode, qrUrl]);

  // 服务器停止或关闭远程访问时清除二维码 / Clear QR code when server stops or remote access is disabled
  useEffect(() => {
    if (!status?.running || !status.allowRemote || !supportsLocalWebuiCredentials) {
      setQrUrl(null);
      setQrExpiresAt(null);
      if (qrRefreshTimerRef.current) {
        clearTimeout(qrRefreshTimerRef.current);
  ...
  }, [status?.allowRemote, status?.running, supportsLocalWebuiCredentials]);
  ...
  const actualPassword = supportsLocalWebuiCredentials ? status?.initialPassword || cachedPassword : null;
  ...
  if (!isDesktop) {
    return (
      <div className='flex flex-col h-full w-full'>
        <BiWorkScrollArea className='flex-1 min-h-0 pb-16px' disableOverflow={isPageMode}>
      ...
      <div className='space-y-12px px-[12px] md:px-[28px]'>
        ...
        */}
                  ...
                  onClick={() =>
                    void talkToButler({
                      prompt: t('settings.talkToButler.prompt.setupRemote', {
                        defaultValue:
                          'Help me set up remote access so I can open BiWork from my phone or over the internet.',
                      }),
                    })
                  }
                >
        ...
        <div className='px-[12px] md:px-[28px] py-14px bg-2 rd-16px'>
          ...
          {!supportsLocalWebuiCredentials && (
            <div className='mb-8px rd-10px border border-line bg-fill-1 px-10px py-8px text-12px text-t-secondary leading-relaxed'>
              {t('settings.webui.oidcManagedCredentials', {
                defaultValue:
                  'WebUI sign-in is managed by FerrisKey/OIDC. Local password reset and QR login are disabled.',
              })}
            </div>
          )}
          ...
          {supportsLocalWebuiCredentials && status?.running && status.allowRemote && (
            <>
