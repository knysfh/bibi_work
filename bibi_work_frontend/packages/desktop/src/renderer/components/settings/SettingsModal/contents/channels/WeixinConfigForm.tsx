...
const WeixinConfigForm: React.FC<WeixinConfigFormProps> = ({ pluginStatus, modelSelection, onStatusChange }) => {
  ...
  const handleApprovePairing = async (code: string) => {
    try {
      const decision = await channel.approvePairing.invoke({ code });
      Message.success(t('settings.assistant.pairingApproved', 'Pairing approved'));
      setPendingPairings((prev) => prev.filter((pairing) => pairing.code !== decision.code));
      if (decision.user) {
        setAuthorizedUsers((prev) => {
          const exists = prev.some((user) => user.id === decision.user?.id);
          return exists ? prev : [decision.user!, ...prev];
        });
      }
    } catch (error) {
  ...
  const handleRejectPairing = async (code: string) => {
    try {
      const decision = await channel.rejectPairing.invoke({ code });
  ...
  const handleRevokeUser = async (user_id: string) => {
    try {
      const revocation = await channel.revokeUser.invoke({ user_id });
      Message.success(t('settings.assistant.userRevoked', 'User access revoked'));
      setAuthorizedUsers((prev) => prev.filter((user) => user.id !== revocation.user_id));
    } catch (error) {
      Message.error(error instanceof Error ? error.message : String(error));
    }
  };
  ...
  const handleLoginWebUI = () => {
    // The old QR-login SSE endpoint is not part of the desktop gateway contract
    // and cannot carry broker Authorization headers with native EventSource.
    setQrcodeDataUrl(null);
    setLoginState('idle');
    Message.error(
      t(
        'settings.weixin.connectorUnavailable',
        'WeChat QR login is unavailable until the local connector runtime is attached'
      )
    );
  };
  ...
  const handleDisconnect = async () => {
    try {
      const status = await channel.disablePlugin.invoke({ plugin_id: 'weixin' });
