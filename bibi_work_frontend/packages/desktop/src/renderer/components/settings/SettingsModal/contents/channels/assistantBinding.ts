 ...
 * Backend normalization is handled before channel settings reads;
 * renderer-side selection must only trust canonical
...
export function getDefaultChannelAssistant(assistants: Assistant[]): Assistant | undefined {
  return assistants.find((assistant) => assistant.source === 'generated') || assistants[0];
}
