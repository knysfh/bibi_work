  ...
  const loadDirectory = useCallback(
    async (dirPath = '') => {
      ...
      try {
        ...
        const data = await httpRequest<DirectoryData>(
          'GET',
          `/api/fs/browse?path=${encodeURIComponent(dirPath)}&showFiles=${showFiles}`
        );
        ...
        const backendMessage =
          isBackendHttpError(err) && err.backendMessage
            ? err.backendMessage
            : isBackendHttpError(err) && err.body && typeof err.body === 'object' && 'message' in err.body
              ? String((err.body as { message?: unknown }).message)
              : null;
        setError(backendMessage || (err instanceof Error ? err.message : 'Failed to load directory'));
      } finally {
        setLoading(false);
      }
    },
