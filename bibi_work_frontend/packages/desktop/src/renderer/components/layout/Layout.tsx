...
import biworkLogo from '@renderer/assets/logo.svg';
...
const Layout: React.FC<{
  ...
  // The "BiWork" wordmark acts as Home / Back-to-Chat, but only from settings routes.
  ...
  const handleBrandHome = useCallback(() => {
    ...
    try {
      target = sessionStorage.getItem('biwork:last-non-settings-path');
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    ...
    const handleCheckUpdate = () => {
      window.dispatchEvent(new CustomEvent('biwork-open-update-modal', { detail: { source: 'tray' } }));
    };
        ...
        <div className='app-shell flex flex-col size-full min-h-0'>
          ...
          <ArcoLayout className={'size-full layout flex-1 min-h-0'}>
            ...
            >
              ...
              >
                <div
                  data-testid='biwork-brand-logo'
                  className={classNames('shrink-0 size-32px relative rd-0.5rem overflow-hidden', {
                    '!size-24px': collapsed,
                  })}
                  onClick={onClick}
                >
                  <img className='block size-full' src={biworkLogo} alt='BiWork' />
                </div>
                {isSettingsRoute ? (
                  ...
                  <div className='text-16px text-t-primary collapsed-hidden font-semibold'>BiWork</div>
                )}
                {isMobile && !collapsed && (
                  <button
                    type='button'
