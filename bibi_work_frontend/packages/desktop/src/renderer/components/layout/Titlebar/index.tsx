...
const Titlebar: React.FC<TitlebarProps> = ({ workspaceAvailable }) => {
  ...
  useEffect(() => {
    if (!isSettingsRoute) {
      ...
      try {
        sessionStorage.setItem('biwork:last-non-settings-path', path);
      } catch {
        // ignore
      }
      return;
    ...
    try {
      const stored = sessionStorage.getItem('biwork:last-non-settings-path');
      if (stored) {
