...
const DEFAULT_REPO = 'knysfh/bibi_work';
const DEFAULT_USER_AGENT = 'BiWork';
const ALLOWED_ASSET_EXTS = new Set(['.exe', '.msi', '.dmg', '.zip', '.deb', '.rpm']);
const CDN_HOST = 'static.biwork.com';
...
const resolveRepo = (requestRepo?: string): string => {
  const envRepo = process.env.BIWORK_GITHUB_REPO?.trim();
...
const sanitizeFileName = (name: string): string => {
  ...
  // Avoid empty names.
  return base || `BiWork-update-${Date.now()}`;
};
