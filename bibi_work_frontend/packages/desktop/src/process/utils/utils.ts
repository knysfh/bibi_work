...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'biwork');
};
 ...
 * Release builds use ~/.biwork; dev builds use ~/.biwork-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.biwork，Dev 模式使用 ~/.biwork-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'biwork');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.biwork'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.biwork-config; dev builds use ~/.biwork-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.biwork-config，Dev 模式使用 ~/.biwork-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.biwork-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[BiWork] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[BiWork] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[BiWork] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[BiWork] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
