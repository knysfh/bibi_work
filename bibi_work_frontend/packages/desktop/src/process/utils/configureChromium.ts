...
// Override via BIWORK_CDP_PORT env variable. Set to "0" to disable.
//
...
function findAvailablePort(preferredPort: number): number {
  ...
  console.log(
    `[CDP] Port ${preferredPort} is occupied by another BiWork instance, scanning range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END}`
  );
  ...
  console.warn(
    `[CDP] All ports in range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END} are used by active BiWork instances, trying ${preferredPort}`
  );
  return preferredPort;
}
...
function resolveCdpPortFromEnv(): number | null | undefined {
  const envVal = process.env.BIWORK_CDP_PORT;
