...
import { peekAccessToken } from '@/common/auth/authTokenBroker';
...
const DEFAULT_WEBUI_PORT = (() => {
  ...
  if (process.env.BIWORK_MULTI_INSTANCE === '1') return 25810;
  return 25809;
})();
...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.BIWORK_PORT ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(process.env.BIWORK_ALLOW_REMOTE || process.env.BIWORK_REMOTE);
  const hostHint = process.env.BIWORK_HOST?.trim();
...
export async function startDesktopWebUI(opts: { port?: number; allowRemote?: boolean }): Promise<DesktopWebUIHandle> {
  ...
  const handle = await startWebHost({
    app: {
      ...
      // getDataPath() returns ~/.biwork[-dev] symlink on macOS to sidestep
