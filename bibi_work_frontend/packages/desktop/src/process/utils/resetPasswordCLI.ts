...
export async function resetPasswordCLI(username: string): Promise<void> {
  ...
  try {
    ...
    if (statusRes.ok) {
      ...
      const authMode = status.auth_mode ?? status.data?.auth_mode;
      if (authMode === 'ferriskey_oidc') {
        throw new Error(
          'Local WebUI password reset is not available in FerrisKey/OIDC mode. Use FerrisKey account recovery or OIDC administration instead.'
        );
      }
    }
