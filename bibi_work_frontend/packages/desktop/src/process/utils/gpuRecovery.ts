...
function envDisablesHardwareAcceleration(): boolean {
  return ['1', 'true', 'yes', 'on'].includes(
    String(process.env.BIWORK_DISABLE_HARDWARE_ACCELERATION ?? '')
      .trim()
      .toLowerCase()
  );
}
...
export function applyGpuRecoveryFlags(): void {
  if (envDisablesHardwareAcceleration()) {
    app.disableHardwareAcceleration();
    console.log('[GPU] hardware acceleration disabled (env BIWORK_DISABLE_HARDWARE_ACCELERATION)');
    return;
  }
