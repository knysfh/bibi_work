...
const STORAGE_PATH = {
  config: 'biwork-config.txt',
  chatMessage: 'biwork-chat-message.txt',
  chat: 'biwork-chat.txt',
  env: '.biwork-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[BiWork] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[BiWork] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[BiWork] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('biwork.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'biwork-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'biwork-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'biwork-chat-history', conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, 'biwork-chat-history', 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[BiWork] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
 ...
 * `crates/biwork-app/assets/builtin-assistants/` and
 * `crates/biwork-app/assets/builtin-skills/`) — neither is synced from
...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[BiWork:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  //    now live in the backend SQLite catalog (see biwork-assistant crate) and
  ...
  try {
    ...
    console.error('[BiWork] Failed to ensure assistant dirs:', error);
  }
