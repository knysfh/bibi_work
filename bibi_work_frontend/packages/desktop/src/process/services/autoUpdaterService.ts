...
const FORCE_DEV_AUTO_UPDATE_ENV = 'BIWORK_FORCE_DEV_AUTO_UPDATE';
...
class AutoUpdaterService extends EventEmitter {
  ...
  constructor() {
    ...
    log.debug('[auto-update] GitHub feed configured', {
      ...
      repository: `${updateFeedOptions.owner}/${updateFeedOptions.repo}`,
  ...
  private ensureDevUpdateConfig(): void {
    try {
      ...
      const devConfig = [
        ...
        'updaterCacheDirName: com.biwork.app',
        '',
      ].join('\n');
  ...
  private moveCwdOutOfInstallDirForWindowsHandoff(): void {
    ...
    try {
      const safeCwd = path.join(app.getPath('temp'), 'biwork-updater-cwd');
   ...
   * while the downloaded archive contains the packaged app (com.biwork.app). Squirrel.Mac
