...
/**
 * Legacy config keys that may still exist on disk from the original desktop implementation.
 *
 ...
 * 会话来源类型 - 标识会话创建的来源
 */
export type ConversationSource = 'biwork' | 'telegram' | 'lark' | 'dingtalk' | 'weixin' | 'wecom' | (string & {});
...
interface IChatConversation<T, Extra> {
  ...
  /** 会话来源，默认为 biwork / Conversation source, defaults to biwork */
...
export type TChatConversation =
  | Omit<
      IChatConversation<
        'acp',
        {
          ...
          /** Compatibility field used by the conversation creation API. */
  ...
  // BiWork backend/crates/biwork-common/src/enums.rs and factory.rs.
...
export interface IMcpServer {
  ...
  health_status?: 'unknown' | 'healthy' | 'unhealthy' | 'unsupported';
  consecutive_failures?: number;
  has_health_error?: boolean;
  created_at: number;
  updated_at: number;
  original_json: string; // 存储原始JSON配置，用于编辑时的准确显示
  /** Built-in MCP server managed by BiWork (hide edit/delete in UI) */
  builtin?: boolean;
}
...
export const BUILTIN_IMAGE_GEN_NAME = 'biwork-image-generation';
export const BUILTIN_IMAGE_GEN_LEGACY_NAMES = ['BiWork Image Generation', BUILTIN_IMAGE_GEN_ID] as const;
