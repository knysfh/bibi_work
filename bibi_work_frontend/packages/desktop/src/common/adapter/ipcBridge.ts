 ...
 * calls routed to BiWork backend. Electron-native operations (window controls,
...
export type WorkbenchBootstrap = {
  ...
  runtime: {
    default_kind: 'deepagents' | 'biwork_cli' | 'disabled';
    supported_kinds: string[];
    python_runtime_kind: 'deepagents';
    desktop_runtime_kind: 'biwork_cli';
  };
  feature_flags: Record<string, unknown>;
  route_ownership: Record<string, unknown>;
  catalog: {
    assistants: Assistant[];
    providers: IProvider[];
    skills: unknown[];
    mcp_servers: IMcpServer[];
    managed_agents: AgentMetadata[];
  };
  assistants: Assistant[];
  providers: IProvider[];
  skills: unknown[];
...
export const workbench = {
  bootstrap: httpGet<WorkbenchBootstrap, void>('/api/workbench/bootstrap'),
};

export type ConversationActiveLeaseResponse = {
...
export type AgentMcpCapabilityTool = {
  id: string;
  name: string;
  description?: string | null;
  schema_hash?: string | null;
  risk_level: 'low' | 'medium' | 'high';
  risk_source: 'server_annotation' | 'name_heuristic' | 'default_untrusted';
  read_only: boolean;
  destructive: boolean;
  selected: boolean;
  stale: boolean;
...
export type AgentMcpCapabilityServer = {
  id: string;
  name: string;
  description?: string | null;
  tools: AgentMcpCapabilityTool[];
};
...
export type AgentMcpCapabilities = {
  ...
  stale_mcp_tool_ids: string[];
  servers: AgentMcpCapabilityServer[];
};

export type PublishAgentMcpCapabilitiesResult = {
  changed: boolean;
  agent_id: string;
  agent_version_id?: string | null;
  browser_enabled: boolean;
...
type WorkspaceSearchResponse = { file: number; dir: number; match?: IDirOrFile };
type WorkspaceSearchResponseHandler = (data: WorkspaceSearchResponse) => Promise<void> | void;

const workspaceSearchResponseHandlers = new Set<WorkspaceSearchResponseHandler>();

function emitWorkspaceSearchResponse(data: WorkspaceSearchResponse) {
  for (const handler of Array.from(workspaceSearchResponseHandlers)) {
    void Promise.resolve(handler(data)).catch((error) => {
      console.warn('[workspace] responseSearchWorkSpace handler failed', error);
    });
  }
}

function summarizeWorkspaceSearchResult(tree: IDirOrFile[]): WorkspaceSearchResponse {
  const children = tree[0]?.children ?? [];
  const file = children.filter((item) => item.isFile).length;
  const dir = children.filter((item) => item.isDir).length;
  return {
    file,
    dir,
    match: children.length === 1 ? children[0] : undefined,
  };
}
...
export const conversation = {
  ...
  createWithConversation: withResponseMap(
    httpPost<TChatConversation, { conversation: TChatConversation }>('/api/conversations/clone'),
    fromApiConversation
  ),
  get: withResponseMap(
  ...
  activeLease: httpPost<ConversationActiveLeaseResponse, { conversation_id: string }>(
  ...
  getWorkspace: {
    ...
    invoke: (async (p: { conversation_id: string; workspace: string; path: string; search?: string }) => {
      ...
      const mapped = fromBackendWorkspaceList(raw, p.workspace, rel);
      if (p.search?.trim()) {
        emitWorkspaceSearchResponse(summarizeWorkspaceSearchResult(mapped));
      }
      return mapped;
    }) as (p: { conversation_id: string; workspace: string; path: string; search?: string }) => Promise<IDirOrFile[]>,
  },
  responseSearchWorkSpace: {
    provider: (handler: WorkspaceSearchResponseHandler) => {
      workspaceSearchResponseHandlers.add(handler);
      return () => {
        workspaceSearchResponseHandlers.delete(handler);
      };
    },
    invoke: (async (data: WorkspaceSearchResponse) => {
      emitWorkspaceSearchResponse(data);
    }) as (p: WorkspaceSearchResponse) => Promise<void>,
  },
  confirmation: {
    ...
    remove: wsEmitter<{ conversation_id: string; id: string; approval_id?: string; call_id?: string }>(
      'confirmation.remove'
    ),
  },
  approval: {
...
export const application = {
  ...
  // The backend reads BIWORK_*_DIR env vars on boot, so it does not own this config.
...
export const fs = {
  ...
  writeFile: httpPost<boolean, { path: string; data: string; workspace?: string; expected_revision?: number | string }>(
    '/api/fs/write'
  ),
  createZip: httpPost<
    boolean,
...
export const mode = {
  ...
  testProvider: httpPost<ProviderHealthCheckResponse, ProviderHealthCheckRequest>(
    (p) => `/api/providers/${p.provider_id}/test`,
    (p) => ({ model: p.model })
  ),
  /**
...
export const acpConversation = {
  ...
  getAgentMcpCapabilities: httpGet<AgentMcpCapabilities, { id: string }>(
    (p) => `/api/agents/${encodeURIComponent(p.id)}/mcp-capabilities`
  ),
  publishAgentMcpCapabilities: httpPut<
    PublishAgentMcpCapabilitiesResult,
    { id: string; mcp_tool_ids: string[]; browser_enabled: boolean }
  >(
    (p) => `/api/agents/${encodeURIComponent(p.id)}/mcp-capabilities`,
    (p) => ({ mcp_tool_ids: p.mcp_tool_ids, browser_enabled: p.browser_enabled })
  ),
...
// ---------------------------------------------------------------------------
// Auth — desktop OIDC lifecycle is owned by the Electron main process
// ---------------------------------------------------------------------------

export const auth = {
  accessTokenChanged: bridge.buildEmitter<{ accessToken: string }>('auth.access-token-changed'),
  loginCompleted: bridge.buildEmitter<{ accessToken: string; authenticated: true }>('auth.login-completed'),
  loginFailed: bridge.buildEmitter<{ message: string }>('auth.login-failed'),
  sessionExpired: bridge.buildEmitter<{ reason: string }>('auth.session-expired'),
};
...
export const cron = {
  ...
  runNow: httpPost<ICronRunNowResponse, { job_id: string }>(
  ...
  onJobExecuted: wsEmitter<ICronJobExecutedEvent>('cron.job-executed'),
};
...
export interface ICronRunNowResponse {
  conversation_id: string;
  run_id: string;
}

export interface ICronJobExecutedEvent {
  job_id: string;
  cron_job_id?: string;
...
export { channel } from './channelBridge';
...
export const hub = {
  ...
  install: httpPost<HubStateChange, { name: string }>('/api/hub/install'),
  uninstall: httpPost<HubStateChange, { name: string }>('/api/hub/uninstall'),
  retryInstall: httpPost<HubStateChange, { name: string }>('/api/hub/retry-install'),
  checkUpdates: httpPost<{ name: string }[], void>('/api/hub/check-updates'),
  update: httpPost<HubStateChange, { name: string }>('/api/hub/update'),
  onStateChanged: wsEmitter<HubStateChange>('hub.state-changed'),
};
