...
type RawFsEntry = { name: string; type: string; full_path?: string; relative_path?: string };
...
export function fromBackendFsEntry(item: RawFsEntry, workspace: string, parentRelPath: string): IDirOrFile {
  ...
  const relativePath = item.relative_path || (parentRelPath ? `${parentRelPath}/${name}` : name);
  return {
    name,
    fullPath: item.full_path || `${ws}/${relativePath}`,
    relativePath,
    isDir,
    isFile: !isDir,
  };
