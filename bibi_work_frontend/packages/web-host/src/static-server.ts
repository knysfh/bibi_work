 ...
 * /login and /logout to bibi_work_backend. All auth goes to the backend;
 * /login and /logout are biwork-auth's top-level paths, the rest live under
...
export type StaticServerOptions = {
  ...
  allowRemote?: boolean;
  /**
   * Optional host-owned header hook for default backend proxy requests.
   * Desktop uses this to attach the main-process OIDC bearer token.
   */
  backendRequestHeaders?: (req: IncomingMessage) => OutgoingHttpHeaders;
  /**
   * Optional host-owned local API hook. The handler owns the response when it
   * returns true; returning false keeps the default backend proxy behavior.
   */
  localApiHandler?: (req: IncomingMessage, res: ServerResponse) => boolean | Promise<boolean>;
  /**
   * Optional host-owned request context wrapper. Desktop uses this for
   * OpenTelemetry server spans while web-host remains telemetry-agnostic.
   */
  requestContext?: (req: IncomingMessage, res: ServerResponse, operation: () => Promise<void>) => Promise<void>;
};
...
const CORS_ALLOWED_HEADERS = 'authorization, content-type, x-requested-with, x-trace-id';
...
function isApiRequestPath(url: string): boolean {
  return url.startsWith('/api/') || url.startsWith('/api?') || url === '/login' || url === '/logout';
}
...
function corsHeadersForRequest(req: IncomingMessage): OutgoingHttpHeaders {
...
function forwardToBackend(
  ...
  return new Promise((resolve) => {
    ...
    const options: http.RequestOptions = {
      ...
      path: req.url,
      method: req.method,
      headers: { ...(backendRequestHeaders?.(req) ?? req.headers), host: `127.0.0.1:${backendPort}` },
    };
    const proxy = http.request(options, (proxyRes) => {
      res.writeHead(proxyRes.statusCode ?? 502, { ...proxyRes.headers, ...corsHeadersForRequest(req) });
      proxyRes.pipe(res);
    });
    proxy.on('error', () => {
      if (!res.headersSent) {
        res.writeHead(502, { 'content-type': 'application/json', ...corsHeadersForRequest(req) });
  ...
  const http_server: Server = http.createServer(async (req, res) => {
    try {
      const handleRequest = async (): Promise<void> => {
        ...
        // /login and /logout are biwork-auth's top-level auth endpoints: proxy them too
        ...
        if (isApiRequestPath(req.url)) {
          ...
          await forwardToBackend(req, res, opts.backendPort, opts.backendRequestHeaders);
          return;
        }
      ...
      if (opts.requestContext) await opts.requestContext(req, res, handleRequest);
