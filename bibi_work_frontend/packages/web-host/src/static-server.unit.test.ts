...
describe('static-server', () => {
  ...
  it('/api/* allows local dev renderer CORS requests', async () => {
    const backend = await startMockBackend((req, res) => {
      res.writeHead(200, { 'content-type': 'application/json' });
      res.end(JSON.stringify({ path: req.url, method: req.method }));
    });
    ...
    const r = await fetch(`${handle.localUrl}/api/auth/oidc/config`, {
    ...
    expect(r.headers.get('access-control-expose-headers')).toContain('x-trace-id');
  });

  it('/api/* answers CORS preflight without calling backend', async () => {
  ...
  it('localApiHandler can intercept local desktop routes before backend proxy', async () => {
    let backendCalled = false;
    const backend = await startMockBackend((_req, res) => {
      backendCalled = true;
      res.writeHead(500).end();
    ...
    handle = await startStaticServer({
      ...
      localApiHandler: (req, res) => {
        if (req.method === 'POST' && req.url === '/api/shell/open-file') {
          res.writeHead(200, { 'content-type': 'application/json' });
    ...
    const r = await fetch(`${handle.localUrl}/api/shell/open-file`, { method: 'POST' });
    expect(r.status).toBe(200);
    expect(await r.json()).toEqual({ success: true, data: null });
    expect(backendCalled).toBe(false);
  });

  it('localApiHandler fallthrough keeps backend proxy behavior', async () => {
    const backend = await startMockBackend((req, res) => {
      res.writeHead(200, { 'content-type': 'application/json' });
      res.end(JSON.stringify({ path: req.url, method: req.method }));
    });
    ...
    handle = await startStaticServer({
      ...
      backendPort: backend.port,
      port: 0,
      localApiHandler: () => false,
    });

    const r = await fetch(`${handle.localUrl}/api/conversations`);
    expect(r.status).toBe(200);
    const json = (await r.json()) as { path: string };
    expect(json.path).toBe('/api/conversations');
  });
  ...
  it('/login reverse-proxies to backend (no local handler)', async () => {
    const backend = await startMockBackend((req, res) => {
      if (req.url === '/login' && req.method === 'POST') {
        res.writeHead(200, {
          ...
          'set-cookie': 'biwork-session=backend-token; Path=/; HttpOnly',
        });
    ...
    expect(r.status).toBe(200);
    expect(r.headers.get('set-cookie')).toMatch(/biwork-session=backend-token/);
  ...
  it('applies host-owned headers to default backend proxy requests', async () => {
    const backend = await startMockBackend((req, res) => {
      if (req.url === '/api/auth/user' && req.method === 'GET') {
        res.writeHead(req.headers.authorization === 'Bearer desktop-token' ? 200 : 401, {
          'content-type': 'application/json',
        });
    ...
    handle = await startStaticServer({
      ...
      backendPort: backend.port,
      port: 0,
      backendRequestHeaders: (req) => ({ ...req.headers, authorization: 'Bearer desktop-token' }),
    });
  ...
  it('keeps the host request context active through backend proxy completion', async () => {
    let contextActive = false;
    let contextObservedByHeaderHook = false;
    const backend = await startMockBackend((req, res) => {
      res.writeHead(req.headers['x-request-context'] === 'active' ? 200 : 500).end();
    });
    stopBackend = backend.close;
    handle = await startStaticServer({
      staticDir,
      backendPort: backend.port,
      port: 0,
      requestContext: async (_req, _res, operation) => {
        contextActive = true;
        try {
          await operation();
        } finally {
      ...
      backendRequestHeaders: (req) => {
        contextObservedByHeaderHook = contextActive;
        return { ...req.headers, 'x-request-context': contextActive ? 'active' : 'inactive' };
      },
    });

    const response = await fetch(`${handle.localUrl}/api/auth/user`);
    expect(response.status).toBe(200);
    expect(contextObservedByHeaderHook).toBe(true);
  ...
  it('/logout reverse-proxies to backend (no local handler)', async () => {
    const backend = await startMockBackend((req, res) => {
      if (req.url === '/logout' && req.method === 'POST') {
        res.writeHead(200, {
          'content-type': 'application/json',
          'set-cookie': 'biwork-session=; Path=/; Max-Age=0',
        });
        res.end(JSON.stringify({ success: true, proxied: true }));
        return;
      }
