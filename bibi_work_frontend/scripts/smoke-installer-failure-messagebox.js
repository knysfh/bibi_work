...
const INSTALLER_ERROR_SCENARIOS = [
  {
    ...
    defineName: 'BIWORK_E_UNINSTALLER_COPY_OR_REBUILD_FAILED',
    code: 'E1001',
    message: 'BiWork could not repair the installed uninstaller.',
    action: 'Close BiWork, restart Windows if needed, then run this installer again.',
    diagnostics:
  ...
  },
  {
    ...
    defineName: 'BIWORK_E_OLD_UNINSTALL_FAILED',
    code: 'E1002',
    message: 'The previous BiWork uninstaller returned an error.',
    action:
  ...
  },
  {
    ...
    defineName: 'BIWORK_E_INSTALL_DIR_REMOVE_OR_LOCKED',
    code: 'E1003',
    message: 'BiWork could not remove or replace the previous installation directory.',
    action: 'Close BiWork and any program using the install folder, then run this installer again.',
    diagnostics: 'scenario=install-dir-remove-or-locked phase=atomic-failed failedPath=install-dir',
  },
  {
    id: 'extract-failed',
    defineName: 'BIWORK_E_EXTRACT_FAILED',
    code: 'E1010',
    message: 'BiWork could not extract the application files correctly.',
    action: 'Download a fresh installer and run it again.',
    diagnostics: 'scenario=extract-failed phase=extract method=zip missing=BiWork.exe',
  },
  {
    id: 'disk-insufficient',
    defineName: 'BIWORK_E_DISK_INSUFFICIENT',
    code: 'E1020',
    message: 'BiWork cannot continue because the target disk does not have enough free space.',
  ...
  },
  {
    ...
    defineName: 'BIWORK_E_CORE_APP_FILES_INCOMPLETE',
    code: 'E1031',
    message: 'BiWork installation is incomplete because a required application file is missing.',
    action: 'Reinstall BiWork or download a newer installer.',
    diagnostics: 'scenario=core-app-files-incomplete phase=verify-required-file missing=resources/app.asar',
  },
  {
    id: 'arch-mismatch',
    defineName: 'BIWORK_E_ARCH_MISMATCH',
    code: 'E1040',
    message: 'Installation package architecture mismatch.',
    action: 'Download the BiWork installer that matches this Windows architecture, then run it again.',
    diagnostics: 'scenario=arch-mismatch phase=arch-check target=x64 actual=arm64',
  },
  {
    id: 'active-installer-conflict',
    defineName: 'BIWORK_E_ACTIVE_INSTALLER_CONFLICT',
    code: 'E1050',
    message: 'Another BiWork installer appears to still be active.',
  ...
  },
  {
    ...
    defineName: 'BIWORK_E_REGISTRY_STATE_INVALID',
    code: 'E1060',
    message: 'BiWork found an invalid previous-install registry state.',
    action: 'Uninstall the old BiWork from Windows Settings, then run this installer again.',
  ...
  },
  {
    ...
    defineName: 'BIWORK_E_ACTIVE_MARKER_WRITE_FAILED',
    code: 'E1070',
    message: 'BiWork could not write the active-installer marker.',
  ...
  },
  {
    ...
    defineName: 'BIWORK_E_INVALID_INSTALL_PATH',
    code: 'E1090',
...
function copyHarnessProject(projectRoot) {
  ...
  writeFileSync(path.join(supportDir, '_sentry-dsn.generated.nsh'), '!define BIWORK_SENTRY_DSN ""\n', 'utf8');
}
...
function readInstallerErrorDefinitions() {
  ...
  const definitions = Array.from(source.matchAll(/!define\s+(BIWORK_E_[A-Z0-9_]+)\s+"(E\d{4})"/g), (match) => ({
    defineName: match[1],
    code: match[2],
  }));
  if (definitions.length === 0) {
    throw new Error('No BIWORK_E_* installer error codes found.');
  }
  return definitions;
}
...
function getInstallerErrorScenarioMatrix() {
  ...
  if (definitions.length !== 11) {
    throw new Error(`Expected 11 installer error code definitions, found ${definitions.length}: ${codes.join(', ')}`);
  }
...
function Find-FailureWindow([string]$Code, [int]$TimeoutSec = 90) {
  ...
  do {
    ...
    foreach ($window in $windows) {
      ...
      if ($text -like "*BiWork installation failed ($Code)*" -or
...
try {
  ...
  foreach ($required in @(
    "BiWork installation failed ($Code)",
    "scenario=$ScenarioId",
    'Suggested action:',
    'Diagnostics:',
    'Installer log:',
...
function createHarnessNsi({ exePath, logPath, projectRoot, scenario }) {
  ...
  return `
Unicode true
Name "BiWork Failure MessageBox Smoke"
...
Section
  StrCpy $INSTDIR "$TEMP\\BiWork-messagebox-smoke"
  StrCpy $BiWorkSessionId "smokembox-${nsisQuote(scenario.code)}"
...
function runHarness({ autoDecline, compileOnly, makensis, scenario }) {
  const { code } = scenario;
  const root = mkdtempSync(path.join(tmpdir(), `biwork-failure-messagebox-${code}-`));
  const projectRoot = path.join(root, 'project');
  const nsiPath = path.join(root, 'biwork-failure-messagebox-smoke.nsi');
  const exePath = path.join(root, 'biwork-failure-messagebox-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `biwork-installer-messagebox-smoke-${code}-${new Date().toISOString().replace(/[-:]/g, '').replace(/\..+$/, '').replace('T', '-')}-log.jsonl`
  );
  const automationPath = path.join(root, 'auto-decline.ps1');
  const reportStatusPath = path.join(process.env.TEMP || tmpdir(), 'biwork-installer-report.json');
  ...
  try {
    ...
    if (autoDecline) {
      ...
      if (autoDecline === 'consent') {
        ...
        if (typeof status.copyText !== 'string' || !status.copyText.includes(`BiWork installer failure ${code}`)) {
