...
export INSTALL_DIR="/tmp/biwork-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/biwork-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/biwork-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/biwork-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/biwork-web"
    exit 1
fi
...
VERSION_OUTPUT=$(biwork-web version 2>&1 || echo "")
