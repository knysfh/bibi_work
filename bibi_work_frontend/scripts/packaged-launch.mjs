...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('BiWork.exe');
    await killProcessByName('BiWork');
  ...
  const env = {
    ...process.env,
    BIWORK_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] BIWORK_EXTENSIONS_PATH: ${env.BIWORK_EXTENSIONS_PATH}`);
