...
function main() {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'biwork-rm-ui-'));
  const installDir = path.join(root, 'install-dir');
  mkdirSync(installDir, { recursive: true });
  const lockedFile = path.join(installDir, 'locked-by-smoke.txt');
  writeFileSync(lockedFile, 'BiWork Restart Manager UI smoke lock\n', 'utf8');

  let locker = null;
  const nsiPath = path.join(root, 'biwork-rstrtmgr-ui-smoke.nsi');
  const exePath = path.join(root, 'biwork-rstrtmgr-ui-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `biwork-installer-smoke-${new Date().toISOString().replace(/[-:]/g, '').replace(/\..+$/, '').replace('T', '-')}.log`
  );
  ...
  const nsi = `
Unicode true
Name "BiWork Restart Manager UI Smoke"
OutFile "${nsisQuote(exePath)}"
RequestExecutionLevel user
SilentInstall normal
!define BIWORK_FALLBACK_LOG "biwork-installer-smoke-fallback.log"
...
Section
  ...
  StrCpy $BiWorkSessionId "rstrtmgrui"
  StrCpy $BiWorkIsUpdated "1"
  InitPluginsDir
  BringToFront

  biwork_query_lockers:
    !insertmacro BIWORK_QUERY_LOCKERS "${nsisQuote(lockedFile)}" $BiWorkLockerResult
