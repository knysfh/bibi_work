...
$logPath = Join-Path $env:TEMP "biwork-installer-smoke-$runId-log.jsonl"
...
@(
  ...
  '{"schemaVersion":1,"event":"failure","session":"smoke","version":"2.1.28","updated":true,"code":"E1003","phase":"atomic-failed","failedPath":"C:\\Users\\huang\\AppData\\Local\\Programs\\BiWork","blockingProcesses":[],"fallbackReason":"restart-manager-no-process","message":"Windows did not identify a specific locking process. Close terminals, editors, and file managers opened in the install folder."}'
...
- File or folder: C:\Users\huang\AppData\Local\Programs\BiWork
...
if ($status.copyText -notlike '*BiWork installer failure E1003*') {
  throw 'copyText does not include failure heading'
}
if ($status.copyText -notlike '*To BiWork Team*') {
