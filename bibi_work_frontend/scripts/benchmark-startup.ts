 ...
 * parsing the electron-log file for [BiWork:ready] / [BiWork:init] /
 * [BiWork:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [BiWork:ready] marks
  ...
  // Parsed from [BiWork:init] marks
  initTotalMs: number;
  // Parsed from [BiWork:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'BiWork-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'BiWork', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'BiWork', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'BiWork', 'logs', `${today}.log`));
  }
...
// Matches: [BiWork:ready] <label> +<ms>ms
// Matches: [BiWork:init]  <label> +<ms>ms
// Matches: [BiWork:process] <label> +<ms>ms
const MARK_REGEX = /\[BiWork:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[BiWork] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[BiWork] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[BiWork] Showing main window')) marks.logs.showingMainWindow = true;
  }
