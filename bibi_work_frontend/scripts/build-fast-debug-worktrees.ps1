param(
  ...
  [string]$WorktreeRoot = (Join-Path $PSScriptRoot '..\..\biwork-build-worktrees'),
...
function New-BuildCommandFile([string]$WorktreePath, [string]$Version, [string]$Dsn) {
  ...
  $lines = @(
    ...
    '$env:BIWORK_DEBUG_AUTO_UPDATE_CURRENT_VERSION = ''' + $Version + '''',
...
function Start-BuildProcess([string]$WorktreePath, [string]$Version, [string]$Dsn, [string]$LogDir) {
  $scriptPath = New-BuildCommandFile $WorktreePath $Version $Dsn
...
function Wait-BuildProcess($Build, [int]$TimeoutSeconds) {
  ...
  $source = Join-Path $Build.worktreePath "out\BiWork-$($Build.version)-win-x64.exe"
  ...
  $target = Join-Path $script:OutputDirResolved "BiWork-$($Build.version)-win-x64.exe"
  Copy-Item -LiteralPath $source -Destination $target -Force
...
try {
  foreach ($version in $buildVersions) {
    $worktreePath = Join-Path $runRoot "BiWork-$version"
  ...
  foreach ($version in $buildVersions) {
    ...
    $build = Start-BuildProcess $worktreePath $version $dsn $runRoot
    if ($Sequential) {
