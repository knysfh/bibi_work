...
/**
 * Simplified build script for BiWork
...
const DEBUG_AUTO_UPDATE_CURRENT_VERSION_ENV = 'BIWORK_DEBUG_AUTO_UPDATE_CURRENT_VERSION';
...
function patchElectronBuilderNsisInstaller() {
  ...
  const copiedUninstallerExecWithLog = `ExecWait '"$uninstallerFileNameTemp" /S /KEEP_APP_DATA $0 --installer-log="$BiWorkSessionLogPath" --installer-session="$BiWorkSessionId" _?=$installationDir' $R0`;
  if (patched.includes(copiedUninstallerExec)) {
    patched = patched.replace(copiedUninstallerExec, copiedUninstallerExecWithLog);
  } else if (
    patched.includes(
      `ExecWait '"$uninstallerFileNameTemp" /S /KEEP_APP_DATA $0 --installer-log="$BiWorkSessionLogPath" _?=$installationDir' $R0`
    )
  ) {
  ...
  const bundledUninstallerOverride = [
    '  ${if} ${FileExists} "$PLUGINSDIR\\BiWork-fixed-uninstaller.exe"',
    '    DetailPrint `BiWork-bundled-uninstaller override source.`',
    '    StrCpy $uninstallerFileName "$PLUGINSDIR\\BiWork-fixed-uninstaller.exe"',
    '  ${endIf}',
  ].join('\n');
  ...
  const inPlaceUninstallerExecWithLog = `ExecWait '"$uninstallerFileName" /S /KEEP_APP_DATA $0 --installer-log="$BiWorkSessionLogPath" --installer-session="$BiWorkSessionId" _?=$installationDir' $R0`;
  if (patched.includes(inPlaceUninstallerExec)) {
    patched = patched.replace(inPlaceUninstallerExec, inPlaceUninstallerExecWithLog);
  } else if (
    patched.includes(
      `ExecWait '"$uninstallerFileName" /S /KEEP_APP_DATA $0 --installer-log="$BiWorkSessionLogPath" _?=$installationDir' $R0`
    )
  ) {
    patched = patched.replace(
      `ExecWait '"$uninstallerFileName" /S /KEEP_APP_DATA $0 --installer-log="$BiWorkSessionLogPath" _?=$installationDir' $R0`,
...
function writeGeneratedSentryDsnInclude(projectRoot) {
  ...
  fs.writeFileSync(
    generatedInclude,
    `!define BIWORK_SENTRY_DSN "${escapeNsisDefineValue(process.env.SENTRY_DSN || '')}"\n`
  );
}
...
try {
  ...
  // 5. Generate the build-time Sentry include.
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const aionRunning = isProcessRunningWindows('BiWork.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (aionRunning || electronRunning) {
        console.log('⚠️  Detected running BiWork/Electron process. Attempting to close...');
        killWindowsProcesses(['BiWork.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running BiWork/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'BiWork.exe');
    ...
    console.log('⚠️  Windows local build failed after BiWork.exe was produced.');
    if (firstError) {
