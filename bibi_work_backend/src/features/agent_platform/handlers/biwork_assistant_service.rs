...
pub async fn biwork_list_assistants(
    ...
    let rows = sqlx::query(
        r#"
        SELECT a.id, a.name, a.description, a.status, a.metadata, a.created_at, a.updated_at,
               COALESCE((
...
pub async fn biwork_create_assistant(
    ...
    let (draft_config, metadata) =
        biwork_assistant_documents(agent_id, None, None, &payload, true)?;
    ...
    let row = sqlx::query(
        r#"
        INSERT INTO agents (id, tenant_id, owner_user_id, name, description, draft_config, metadata, status)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        RETURNING id, name, description, status, metadata, draft_config AS config, created_at, updated_at
        "#,
    )
    .bind(agent_id)
    .bind(ctx.tenant_id)
    ...
    .bind(name)
    .bind(payload.get("description").and_then(Value::as_str).map(str::to_string))
    .bind(&draft_config)
    .bind(&metadata)
    .bind(status)
    .fetch_one(&mut *tx)
    .await?;
    publish_biwork_assistant_version(&mut tx, ctx.tenant_id, agent_id, &draft_config).await?;
    tx.commit()
        .await
...
pub async fn biwork_import_assistants(
    ...
    for assistant in assistants {
        ...
        let agent_id = requested_id
            ...
            .unwrap_or_else(Uuid::new_v4);
        let (draft_config, metadata) =
            biwork_assistant_documents(agent_id, None, None, assistant, true)?;
        let mut tx = state.connect_pool.begin().await?;
        sqlx::query(
            r#"
            INSERT INTO agents (id, tenant_id, owner_user_id, name, description, draft_config, metadata, status)
        ...
        .bind(assistant.get("description").and_then(Value::as_str).map(str::to_string))
        .bind(&draft_config)
        .bind(&metadata)
        .execute(&mut *tx)
        .await?;
...
fn assistant_from_row(
    ...
    let raw_agent_source = value_string(&metadata, "source");
    ...
    let selectable = status != "disabled" && runtime_runnable;
    ...
    let engine_agent_id =
        value_string(&config, "engine_agent_id").unwrap_or_else(|| id.to_string());
...
pub(super) fn biwork_assistant_documents(
    ...
    if let Some(engine_agent_id) = trimmed_string(payload, "agent_id") {
        config_object.insert(
            "engine_agent_id".to_string(),
            Value::String(engine_agent_id),
        );
    } else if is_create {
        config_object.insert(
            "engine_agent_id".to_string(),
    ...
    let has_runtime_kind = config_object
        .get("runtime")
        .and_then(Value::as_object)
        .and_then(|runtime| runtime.get("kind"))
        .is_some();
    if !has_runtime_kind {
        config_object.insert("runtime".to_string(), json!({ "kind": "deepagents" }));
    }
...
async fn update_biwork_assistant_record(
    ...
    let row = sqlx::query(
        r#"
        SELECT name, description, draft_config, metadata, status
        FROM agents
        WHERE id = $1 AND tenant_id = $2 AND deleted_at IS NULL
        "#,
    )
    ...
    let (draft_config, metadata) = biwork_assistant_documents(
        agent_id,
...
async fn publish_biwork_assistant_version(
    ...
    sqlx::query(
        ...
        INSERT INTO agent_versions
            (tenant_id, agent_id, version_label, config_snapshot, schema_hash, status)
